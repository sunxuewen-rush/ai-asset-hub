...
> Date: 2026-09-16
> Updated: 2026-09-17（**v0.23：原型删除闭环**（用户授权「授权删原型」）—— ① 删前核实代码零引用（import 0 · `main.tsx` 挂载 0）② 删除 `apps/web/src/pages/__proto/`（含 `M4b2UiProto.tsx` 29,307 B）⇒ 代码侧 `__proto` 零命中 ③ §3 审计表 F3 已闭环 · T15 落地行同步；**v0.22：收尾二次审计 + Status 收口 → ✅ 完成** —— §3 由占位改为实填十一维 findings 表（修 4 / 维持 7 · 无未决项）· Status → ✅ 完成（T1-T15 全绿 · 出口五件全绿））
> **头部口径**同批 design（2026-09-18 起）：只留最近 1-2 版 · 不复述验收数字；完整历史见 **§5 修订记录**。
> SSOT：实测值与断言数 → `docs/smoke/` 证据文件 · 自检分 → 批 design **§12.2** · Task 状态 → **本文件 §2**。
> Status: **✅ 完成（2026-09-17）**（**T1-T15 ✅ 全绿** · 出口五件全绿 · **二次收尾审计无未决项**（§3）· UI 视觉重做（T11-T15）已落地 · 2026-09-17 五组补充改动（官方结构 / SiteHeader 对齐 / 图标态闭环 / 登录落点改首页 / 滚动条收敛）均已登记批 design v1.25-v1.28）
...
- **Assert**:
  ...
  ④ 成功链：`invalidateCache()` → 重取 `/me` → `navigate(next ?? '/')`（**无 `next` 落首页**——2026-09-17 用户拍板；原 `/dashboard`）
...
- **出口五件状态**：① 批 design 8 维 ≥9（定稿 **9.44** / converge **9.50**）✅ ② T1–T15 全绿 ✅（T11–T15 = UI 视觉重做轮 · 2026-09-17 落地）
...
## 3. 整体审计（收尾 · 2026-09-17 · **二次审计**）

> 口径来源：**十一维**扫描（`docs/00` §7 ② · M4b-1 plan §3 先例）。
> **背景**：本批首次审计在 **T10（2026-09-16）** 完成（findings 见 T10 段）；其后又落 **T11-T15（UI 视觉重做）**
> 与 **2026-09-17 的五组补充改动**（官方结构 · SiteHeader 对齐 · 图标态闭环 · 登录落点 · 滚动条收敛）
> ⇒ 按批间门「状态表是唯一源」的要求，**补做一次覆盖最新状态的收尾审计**。

| # | 维度 | 发现（实测） | 处置 |
|---|------|--------------|------|
| F1 | 死导出 | M4b-2 相关件全部有外部消费者（`PageHeader` 3 · `UserMenu` 4 · `ComingSoon` 3 · `RoleGuard` 4 · `roles` 5 · `AuthProvider` 9 · `next` 20 个文件）⇒ **0 死导出** | 维持 |
| F2 | i18n 键 | 实测 **11 组 / 222 键 / 双语双向差集 0**（`bun -e` 真 import 两字典） | 维持 |
| F3 | 已删件 / 原型残留 | `AuthLayout` 34 处 = 文档 31 + **代码 3 处均为注释说明「已删件」（非引用）**；`M4b2UiProto` / `__proto` 本体 **已于 2026-09-17 删除**（用户授权）⇒ 代码侧零命中（余下命中全在文档的史实记载内） | **已闭环**（删除完成） |
| F4 | 越轴任意值 | `h-[42px]`（登录/设备提交按钮 · **在轴**设计值）· `w-[336px]`（登录表单列）· `text-[11px]`/`[13px]`（§4.4 首档，在轴）⇒ 无出轴 | 维持 |
| F5 | token 消费者 | `--scroll-*` 4 处（1 使用 + 3 注释）· `--header-height` 6 处（1 定义 + 1 使用 + 注释）⇒ **无孤儿 token** | 维持 |
| F6 | 注释腐化 | ① `aih-theme.css` 旧注释仍写「6px 细蓝滚动条逐条搬」（该机制已删）② 证据文件轮次 3 的「回退 AIH 6px」结论**已被当日后续轮次推翻** | **修**（① 改注释 ② 加推翻指针） |
| F7 | 文档数字实测 | 「49 PASS / 0 FAIL」「9.94」「222 键」「48/47」「1184」等与本轮实测一致；但**「6px 细蓝」旧口径仍在 3 处历史行**（主 design v1.43 · docs/00 v1.64 · 证据轮次 3） | **修**（三处加「已被推翻」指针，史实值保留） |
| F8 | 官方件硬规则 | `components/ui/shadcn/` 与 HEAD **0 差异** ⇒ 官方件源码**从未改造**（本轮全部改动在消费侧 className / 内联 style / token） | 维持 |
| F9 | 既有登记项状态 | T10 段三处审计发现（行数声明 / F4 关闭 / 旧口径指针）状态未变；**F4 真因**在本轮进一步定位（官方件规则在 tw 4.3.3 未生成 ⇒ 状态驱动等效实现，见批 design v1.25） | 回填 |
| F10 | 旧口径 / 术语指针 | 四份文档中「待深挖 / 待补 / 评审中 / 待拍板 / 待用户」共 56 处，逐条抽查**均为历史版本行内的史实描述**（非现行待办） | 维持 |
| F11 | 生成物 / 出轴 | `apps/server/dist/` 陈旧产物存在但 `bun run test` 不拉 `dist`（既有登记）；`drizzle/meta` 已在 biome 排除（AGENTS.md 硬规则） | 维持（既有登记） |

**处置合计**：修 **4 处**（F6 两处 + F7 三处，落 4 个文件）· 维持 **7 类** —— **无未决项** ✅

**出口五件状态（终态）**：① 批 design 8 维 ≥9（定稿 **9.44** / converge **9.50** / 收尾重评 **9.88**）✅
② **T1-T15 全绿** ✅ ③ 五门禁 exit 0 ✅ ④ dogfood **36/36 + chain-smoke PASS + 用户认可** ✅
⑤ **整体审计无未决项**（本节）✅ —— 证据 `docs/smoke/2026-09-16-m4b2-auth-shell.md` + `docs/smoke/2026-09-17-m4b2-ui-redo.md`
...
| 版本 | 日期 | 作者 | 变更 |
|------|------|------|------|
| v0.23 | 2026-09-17 | sunxuewen-rush | **原型删除闭环**（用户授权「授权删原型」）—— ① 删前核实：`apps/web/src` 内 `__proto` / `M4b2UiProto` 的 import 引用 **0** · `main.tsx` 挂载 **0** ② 删除 `apps/web/src/pages/__proto/`（含 `M4b2UiProto.tsx` 29,307 B）⇒ 删除后代码侧 `__proto` **零命中** ③ §3 审计表 **F3 行处置 → 已闭环**；T15 落地行同步（验收与门禁见 `docs/smoke/` 证据文件） |
| v0.22 | 2026-09-17 | sunxuewen-rush | **收尾二次审计 + Status 收口 → ✅ 完成** —— ① §3 由「待 T10」占位改为**实填二次审计 findings 表**（十一维 · 修 4 / 维持 7 · **无未决项**）② Status → **✅ 完成**（T1-T15 全绿 · 出口五件全绿）③ 背景：T10 首次审计后又有 T11-T15 与五组补充改动 ⇒ 按「状态表是唯一源」补审 |
| v0.21 | 2026-09-17 | sunxuewen-rush | **登录默认落点改首页**（用户拍板「按推荐」）—— ① `pages/Login.tsx` 的 `destination` 默认值 `/dashboard` → `/`（唯一改点；`next` 仍优先，被拦截后登录回原页不变）② `RoleGuard` 无权限弹回点**保持 `/dashboard`**（权限语义的安全落点）③ 反向守卫与成功跳转共用同一值 ⇒ 行为天然一致（断言数见证据文件） |
| v0.20 | 2026-09-17 | sunxuewen-rush | **官方结构 + 官方 SiteHeader 对齐 + 图标态闭环**（T11 补记四 · 用户拍板「对齐官方骨架」）—— ① **结构改官方形态**：`SidebarProvider > SideNav + SidebarInset`（顶栏入 Inset ⇒ **不再全宽** · 侧栏回 `inset-y-0` · 官方 container `border-r` **1px** · **AIH 覆盖点 2 → 1 处**）② **顶栏照官方 `dashboard-01` SiteHeader 对齐**（`--header-height` 变量 / 内层容器 / `Separator mx-2` / 标题改 **`<h1>`** / `shrink-0`+`transition` / **图标态保持 58**）+ 品牌移入**侧栏顶部** `SidebarHeader` ③ **图标态闭环**（官方件规则在 tw 4.3.3 未生成 ⇒ 受控 open + **状态驱动**宽度 256↔48）④ 收起态**单层底色** · hover 提示**统一纯中文** ⑤ 滚动条**方案 ②**（`scrollbar-gutter: stable`） |
