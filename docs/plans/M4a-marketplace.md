...
> Date: 2026-09-09
> Updated: 2026-09-09（**v0.16：M4a 市场门户实现完成归档** —— T1-T19 全绿、业务闭环落地；详见 **§5 修订记录**）
> **头部口径（2026-09-18 起）**：只留最近 1-2 版 · 不复述历史与验收数字；完整历史见 **§5 修订记录**。
> SSOT：实测值与断言数 → `docs/smoke/2026-09-09-m4a-t18.md` 等证据文件。
> Status: ✅ 完成（M4a 收官 2026-09-09——T1-T19 全绿 + 全仓四门禁绿 + converge 重评 ≥9；浏览器 dogfood 截图与联调记录 docs/smoke/2026-09-09-m4a-t18.md；00 §5 M4a ✅、M4b 管理后台拆待办）
...
✅ 完成（commit `893b729`；分型探测 5 例 + md 渲染断言 + manifestFields 数组过滤修复）
...
✅ 完成（commit `9f2b7ca`；树纯函数 + SSR 结构 15 断言；修复：macOS 大小写 FS（FileTree.tsx vs
fileTree.ts → fileTreeNodes.ts）、noStaticElementInteractions → 遮罩真 button 化）
...
✅ 完成（commit `6958480`；板块 E 收官——diff SSR 12 断言（徽章三态/+N−M/前缀/YANKED 预拦截/单版本空态）；修复录：pair 归一纯函数化/行 key 去 index/label htmlFor）
...
✅ 完成（2026-09-09；全仓四门禁真跑绿——typecheck + 538 tests + lint（server 债清后 0 错）+
build；converge 回查：版本头/修订记录/引用链/状态全同步（00 v1.12 §5 M4a ✅ M4b ⬜；07 v1.2
资源落地注记）；design 无腐化点（G8 契约未写 invalid-400 句——T6 真相注记覆盖）；8 维重评 ≥9；
18 维自检逐维留档见下方；里程碑出口记录并入此 commit）
...
| 版本 | 日期 | 作者 | 变更 |
|------|------|------|------|
| v0.16 | 2026-09-09 | sunxuewen-rush | M4a 收官后 18 维终审修复注记——🟡1 筛选/搜索 page 回落 1、🟡2 中心双计数文案区分、🟡3 详情下载受控反馈、🟡4 冒烟脚本收编 docs/smoke/scripts（重放验证 22/22 PASS）；详见 docs/smoke/2026-09-09-m4a-t18.md §7。 |
| v0.15 | 2026-09-09 | sunxuewen-rush | T19 收敛完成回填——全仓门禁绿 + converge 回查 + 8 维重评 ≥9 + 18 维自检留档；00 §5 M4a ✅/M4b ⬜ 注记 v1.12；07 v1.2 资源落地注记。 |
| v0.14 | 2026-09-09 | sunxuewen-rush | T18 完成回填——commit `83be7f6` fix(web) useApi StrictMode abort 守卫（真 bug：React 19 dev 双跑 effect + cleanup 即 abort → 双跑第二波全 AbortError；页内 fetch 包装实证 + StrictMode 摘除对照——修复改 alive 守卫兜底）+ `2fbc303` chore(server) biome 格式/lint 债全清（41 文件排版 + 20 处断言机械守卫改写——turbo 缓存掩盖的存量债；538 tests 全绿 + 真实链复验证语义零变）；联调记录 docs/smoke/2026-09-09-m4a-t18.md（seed 直插 2 资产 3 版本 + 全链 29/29 + Edge headless dogfood 26/26 零 console 错 + 截图 6 张入档）。 |
| v0.13 | 2026-09-09 | sunxuewen-rush | T17 完成回填——commit hash + 板块 E（T14-T17）全部 ✅；diff 12 断言（徽章三态/统计/前缀/YANKED 预拦截/单版本空态）+ 修复录（pair 归一纯函数化/行 key 去 index/label 关联）。 |
| v0.12 | 2026-09-09 | sunxuewen-rush | T16 完成回填——commit hash；文件树/预览对话框 15 断言 + 修复录（macOS 大小写 FS 冲突/遮罩 button 化）。 |
| v0.11 | 2026-09-09 | sunxuewen-rush | T15 完成回填——commit hash；总览分型探测/md 渲染/摘要过滤断言 + manifestFields 数组过滤 bug 修复。 |
| v0.10 | 2026-09-09 | sunxuewen-rush | T14 完成回填——commit hash；详情壳两波编排 + tab 语义 SSR 断言。 |
| v0.9 | 2026-09-09 | sunxuewen-rush | T13 完成回填——commit hash；卡/筛选 SSR 断言 + curl 契约实测（type/q/label 参数名）。 |
| v0.8 | 2026-09-09 | sunxuewen-rush | T12 完成回填——commit hash；三中心参数化 SSR 冒烟 9 断言（type 注入矩阵 + URL q 还原）。 |
| v0.7 | 2026-09-09 | sunxuewen-rush | T11 完成回填——commit hash；首页 SSR 冒烟 12 断言（zh/en 双语）。 |
| v0.6 | 2026-09-09 | sunxuewen-rush | T10 完成回填——commit hash + 板块 B（T7-T10）全部 ✅；SSR 冒烟 12 断言留证。 |
| v0.5 | 2026-09-09 | sunxuewen-rush | T9 完成回填——commit hash；原子组件 SSR 冒烟 17 断言留证。 |
| v0.4 | 2026-09-09 | sunxuewen-rush | T8 完成回填——commit hash + compare.ts 建文件注记（§4.2 树对齐，plan Files 未列）；T6 漂移注记（from=to 实测 200 空 files——测试为真相，非 plan 所写 400；version_compare_invalid 未落地）。 |
| v0.3 | 2026-09-09 | sunxuewen-rush | T7 完成回填——依赖版本入档 + commit hash；注记 I18nProvider 装配归 T8（i18n 层属 T8 交付物）。 |
| v0.2 | 2026-09-09 | sunxuewen-rush | 板块 A（T1-T6）执行完成——任务状态回填 ✅ + commit hash；Status 转执行中。v0.1 修复：R8/R9 错误码注册/T14 YANKED 断言/T6 依赖声明/T11 v0.8 规格/引用链 v0.8 |

> 注：本表由 **2026-09-18 文档体检**从版本头史料**回填**（该 plan 此前**无独立修订表**，属体检 P0-③）；
> 日期按本计划执行期（2026-09-08 ~ 09-09）**推定**，变更内容**逐条取自原版本头、未改写**。
