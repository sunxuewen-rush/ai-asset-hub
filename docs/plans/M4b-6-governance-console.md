...
> Date: 2026-09-23
> Updated: 2026-09-24（**v0.15：① 重挂归属已证（G16）** —— 新增 dogfood **G16**（6 条）：端点三态对比（原子行消失 / 零计数目标新父 count 严格等值 / 上卷守恒 / `rankings` 同面）+ 真页图例行数对齐 + 复原逐字段回落 + **反证**（停重挂 ⇒ 3 条 FAIL）⇒ 全量 **102 → 108 PASS / 0 FAIL**；**口径纠正**（去重口径下「新父 += 原子」不成立 ⇒ 取零计数目标做严格等值）；证据 §8 item 9 闭合）
> Updated: 2026-09-24（**v0.14：F219 跨批断言修复 + 夹具复位** —— ① `m4b4` dogfood：`G20-9` 行数写死 6 → **端点真值** · `G6` 「活跃」→ 真值文案「**已上线**」 ② `m4b4-seed-assets` 复位（`m4b4-seed-skill@1.0.0` 曾被跑测打成 `YANKED` + latest 指针空）⇒ **83/6 → 89/0** ③ 固化「**seed → dogfood 成对跑**」 ④ 造数复位三脚本 ⇒ 零回归 **21 → 2 条**（余 m4a 2 条需两份可比版本））
> Updated: 2026-09-24（**v0.13：F218 一级可重挂（用户拍板「②」）** —— ① 服务端 `updateLabel` 删「一级不可降级」硬拒 → **安全重挂**（目标必须一级 + **自身无子级**；守卫在 `resolveParent` 之后）② 前端父级下拉**放开**（候选 = 一级且**排除自身**；仅「有子级」禁用 + 说明）③ i18n `parentHint` 改写 + `parentLockedHint` → `parentHasChildrenHint`（键数不变）④ **规范层 `docs/06` v1.7**（明写相对兄弟仓 SkillHub 契约的偏离 + 理由）⑤ 用例 F218（5 步）+ dogfood **G8.8/G8.9 改写 + G8.11/G8.12**（含反证）⇒ 断言 **100 → 102**）
> Status: ✅ **已落地（T1–T10）· 看板维重做（T6⁺ · 2026-09-23）** —— 批 design **v0.41**· 四页真页 + 服务端 8 项 + 迁移 0014 · dogfood **G1–G16 = 108/0**（G16 后）· 逐 Task 落地记录见 §7（含 F203–F218）
