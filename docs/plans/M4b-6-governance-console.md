...
> Updated: 2026-09-24（**v0.12：F217 编辑失败不再静默 + F216 已修** —— ① **F217**：两个写函数补 `catch`+`toast` · 一级标签父级下拉禁用 + 说明 · 补齐 10 个 `label.*` 错误文案（i18n 527 → 536）· dogfood **G8.8–G8.10**（含反证）② **F216**：`invalidateLabelCaches()` 三前缀 + 四写函数挂载 ⇒ 断言 **92 → 97 → 100**）
> Status: ✅ **已落地（T1–T10）· 看板维重做（T6⁺ · 2026-09-23）** —— 批 design **v0.41**· 四页真页 + 服务端 8 项 + 迁移 0014 · dogfood **G1–G15 = 102/0**（F218 后）· 逐 Task 落地记录见 §7（含 F203–F218）
