...
| 4 | **i18n** | 新建 `submissions`（**25 键**）· `tokens`（**47 键**）· `errors` **+7 码** · `common` **+1** ⇒ **净增 80 键**（2026-09-17 订正：`error.load` ×2 删除） |
...
② 键数实测：submissions **26** · tokens **48** · errors **21 → 28** · common **8** ⇒ **净增 80**（**v0.10 实测**；原文 41/22/74 为过期口径；**2026-09-17 再订正：`error.load` ×2 删除 ⇒ 25 / 47 / 净增 80（v0.13）**）
...
| 收口 3.1/3.2 | （本批收口提交）| — | 侧栏高亮 **4 路径真浏览器实测** + 键数**实测** 25/47/28/8（zh=en 对称） | T9④ 工作台精确匹配（拍板 ①）· T8③ `error.load` ×2 删除（拍板 ②）· 断言 **69/69** |
...
> **v0.13 复评（2026-09-17，两条未闭环断言收口后）：8 维 9.89** —— 完整性 **9.9**（收口台账行 + 键数订正落点 2 处 + 断言 69/69）·
> 一致性 **10**（与批 design **v1.21** 对照；键数 25/47/28/8 实测一致）· 清晰度 **9.8** · 可实施性 **9.8** · 设计纯粹性 **9.6** ·
> 边界覆盖 **9.8**（Q6 改判留痕 + 替代出口写明）· 实施精度 **9.9**；跨平台 N/A（分母 7）。**换靶角度** = 未闭环项清零 + 数字重测。

## 8. 修订记录

| 版本 | 日期 | 作者 | 变更 |
|------|------|------|------|
| v0.13 | 2026-09-17 | sunxuewen-rush | **两条未闭环断言收口 + 键数订正** ① T8③ `error.load` ×2 删除（拍板 ②；Q6 改判）⇒ 键数 **25/47/28/8**、净增 **80**（§1 件表 + §9 断言 ② 订正）② T9④ 工作台精确匹配（拍板 ①）③ **T1-T10 断言 69/69 全闭环** ④ 8 维 **9.88 → 9.89**；与 design **v1.21** 同源 |
