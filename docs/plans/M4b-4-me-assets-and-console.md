...
| 2 | 我的资产 `/dashboard/assets` | 占位页 → **九列真页**（状态筛选**显式 `status=ALL`** + q 搜索 + 分页 + **无行菜单**；操作列 = 「快速预览」图标钮） |
| 3 | 资产管理抽屉 | 新件（右侧 `Sheet` · 宽 **560** · **纯预览**：描述 / 标签+- / 统计行 / 「完整详情 ↗」） |
...
> **登记缺口（本批显式不做）**：抽屉**无「提交审核」入口**（提审端点已在，但上传无 UI ⇒ 链路接不上；
...
| `components/console/LabelCard.tsx` | 标签卡（与抽屉「标签+-」段**同件**）：chips（`displayName`）+ × + 添加标签（`Popover`+`Command`）+ 特权标签（仅超管） |
| `components/console/asset-stats.tsx` | 下载/收藏展示小件（图标 + `compactCount` · **图标一律无色** · 收藏态表达可开关）—— 列表列 / 抽屉统计行 / 详情页元信息卡**三处共用** |
