...
> Updated: 2026-09-21（**v0.44：`j4` 索引迁移落地** —— ① schema **+3 部分索引**（`idx_asset_newest`/`idx_asset_downloads`/`idx_asset_stars` · 谓词 `status='ACTIVE'`）· 生成 **`0013_good_wolverine.sql`**（恰 3 条 `CREATE INDEX`）· `\d asset` 实测 **6 条** · 二次 migrate **幂等** · 实验**残留 0** ② **迁移前对照实测**（20 万行 · `INSERT 0 200000` 自证）：三档 **8.666/7.944/9.013 → 0.020/0.030/0.030 ms**（≈265–433×）· **B5 闭合**（`status+type` → **0.016 ms** 前缀命中）· **B3 回填**（owner+ALL **不受益**）· `count(*)` 不受益 · 参数化命中 · 建索引 **93.1 ms** ③ 三条新发现：**F116**（实验须自证数据入表 —— int4 溢出致空表假象）· **F117**（小表 `Seq Scan` 是规划器自由选择 ⇒ 断言改两段式）· **F118**（`j3` 连带陈旧引用 —— 档名级扫描）④ 依据 = 批 design **v1.15**（8 维 **9.35**））
