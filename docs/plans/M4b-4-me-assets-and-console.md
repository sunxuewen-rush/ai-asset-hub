...
> Updated: 2026-09-21（**v0.40：实现期 `T11-j · j1` 开工回填** —— ① §3 `j1` 步骤 5 补 `selectionLabels`（**F110** · 选择列 **a11y 名** · 可选 · 缺省不写 `aria-label`）+ 标注官方 `checkbox` 件**已装** ② §7.6 登记 **F109**（`m4b3` dogfood **`G6` 假失败** —— 探针把**占位符 `—` 当身份** · 实库双证 2 × 4 行 ⇒ **判据订正** · 跨批交付物改动 · 先例 F69）· **F110**（design **F-10** 增补 · 用户「认」）③ §7.5 `T11-j` → **🔵 执行中**（`j1` ✅ **9.34**）④ 实测：四门禁 exit 0 · 探针 **30/0** · 对照实验（干净 HEAD 38/1 ⇄ 带 `j1` 38/1 **逐项相同**）= **零回归** ⑤ 依据 = 跨批 design **v1.12**（8 维 **9.31**））
> Updated: 2026-09-21（**v0.39：提交前检查轮订正 F108 + 头部卫生复核** —— 换靶 =「**把未提交改动当提交件审**」：j3 的「**6 处**」与自身枚举**对不上** ⇒ 订正 **动 4 用例 + 保留 4 用例**（与 design **E20** 同源）；被删头部 **29 条 token 级零丢失**复核；重评 **9.50 → 9.49**（8 维 75.9 ÷ 8 · 准确性 9.5 → 9.4））
> 更早版本（**v0.1–v0.38**）摘要见 **§9 修订记录** —— 头部只留最近 3 条（2026-09-21 头部卫生 · 用户拍板）
> Status: **✅ 全绿（2026-09-18）**（**T1–T16** 全绿 · **T8 作废**；T11 收尾已完成：造数 + G1–G19 **53 PASS / 0 FAIL** + 五门禁 exit 0 + 证据回填；**v0.20 验收期第二笔 T11-e 已实现**（G20/G21 新增 ⇒ **77 PASS / 0 FAIL**）· 文档回写完成 · **已提交推送**（`39d1b21` · `9219069`））；**验收期第三笔 T11-f（资产排序）2026-09-20 全绿落地**（`f1 9.35 · f2 9.46 · f3 9.46 · f4 9.47` · 服务端 `sort`/`dir` 契约变更 · 零迁移）· **T11-g（验证效率）全绿**（代码 18 维 `9.49`）· **验收期第六笔 T11-j（表格族统一 · 门户笔）执行中**（`j1` 已落码 —— 统一件 **416** 行 · 代码 18 维 **9.34** · 四门禁 exit 0 · 探针 30/0 · **待提交**）· **三笔已提交推送**（`72e48db` / `434043b` / `ced0d6a`））；其中 **T8 作废**；**执行序 = T15 最先** —— star 能力先行，再消费面）—— 上游批 design 已**定稿**（8 维**以批 design 版本头为准**（现行 **9.69**）；v1.7 原型评审收口）
...
**j2 断言 / 门禁**：① 7 列 + 行内**恰 1 个 button**（`Eye`）② 点操作钮 ⇒ `pathname = /assets/:slug` ③ 表头**无底纹**（`getComputedStyle` 背景透明）④ `table-fixed` + 名称列 22% 生效 ⑤ 载态表头含排序钮 ⑥ web 四门禁 exit 0
...
2. `assets/service.test.ts` 按三档重算 —— **动 4 个用例**（`:280` name 档 **删** · `:291` author 档 **删** · `:301` 内的 `:306` name **换档**（`:319` downloads asc 半保留）· `:341` 两档零 join 循环 **整条作废**）+ **保留 4 个用例**（`:254` 缺省 · `:260` downloads · `:270` stars · `:330` 静默回落）
...
| **T11-j**（表格族统一 · **门户笔**：统一件 + 门户 preset + 排序档收敛 + 3 条部分索引） | 🔵 **执行中**（`j1` ✅ · `j2`–`j5` ⬜） | `j1` **9.34** | 依据 = 跨批 design **v1.12**（8 维 **9.31**）· 步序 **j1→j5** · 含 **1 次迁移**（`0013_*`）· 零新依赖 · 新建件 1（官方 `checkbox` · **已装 26 行**）· **`j1` 门禁** = `typecheck` / `lint` / `format:check` / `build` **exit 0** + 探针 **30/0** + dogfood 对照（干净 HEAD **38/1** ⇄ 带 `j1` **38/1** 逐项相同）**零回归** · **待提交**（时机待口令）|
