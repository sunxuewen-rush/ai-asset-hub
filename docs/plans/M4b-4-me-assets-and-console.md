...
| **T11-j**（表格族统一 · **门户笔**：统一件 + 门户 preset + 排序档收敛 + 3 条部分索引） | ✅ **全绿（门户笔收口 · 2026-09-21）** | `j1` **9.34** · `j2` **9.48** · `j3` **9.47** · `j4` **9.53** · `j5` **9.49** | 依据 = 跨批 design **v1.16**（8 维 **9.38** · 门户笔设计侧收口）· 步序 **j1→j5** · 含 **1 次迁移**（`0013_*`）· 零新依赖 · 新建件 1（官方 `checkbox` · **已装 26 行**）· **`j1` 门禁** = 四门禁 **exit 0** + 探针 **30/0** + dogfood 对照（干净 HEAD **38/1** ⇄ 带 `j1` **38/1**）**零回归** ⇒ 已提交 `ce969cb`｜**`j2` 门禁** = 四门禁 **exit 0** + 运行期探针 **10/0** ⇒ 已提交 `0b2d2f2`｜**`j3` 门禁** = CI 同序七步全绿（`test` **559 pass / 0 fail**）⇒ 已提交 `58ff23e`｜**`j4` 门禁** = `\d asset` **6 条** · 二次 migrate 幂等 · 实验**残留 0** · 迁移前对照实测（20 万行 7 行数字）⇒ 已提交 `583d2aa`｜**`j5` 门禁** = `m4b4` **89/0** · `m4a` **60/0** · `m4b3` **39/0** · CI 同序八步全绿 · 证据回填完成 ⇒ **门户笔全绿**（下一步 = `T11-k` 控制台笔 · 独立一笔）|
