...
> Updated: 2026-09-21（**v0.38：验收期第六笔「表格族统一」立项（T11-j / T11-k · 两笔）** —— 用户 2026-09-21「**任务清单落点--追加到M4b-4**」⇒ ① §2 Task 总览增 **T11-j（门户笔）** / **T11-k（控制台笔）** + 执行序（`j1→j5` 验绿 ⇒ `k1→k2`）② **§3 增两块明细**（j1–j5 / k1–k2 · 各带 文件 / 动作 / 验收断言 / 门禁）③ **§1 非目标 + §4 门禁的「本批零迁移」口径订正**（本笔**含 1 次迁移** —— 3 条部分索引 `0013_*`）④ §6 **+R7**（迁移写放大 / 锁窗口）⑤ §7.5 增两行 · 依据 = 跨批 design `2026-09-21-table-family-alignment-design.md`（**v1.9 定稿 · 8 维 9.31**） · 同批**头部卫生**：只留最近 3 条
...
> 更早版本（**v0.1–v0.36**）摘要见 **§9 修订记录** —— 头部只留最近 3 条（2026-09-21 头部卫生 · 用户拍板）
...
5. **预置能力（件内有、页面不挂）**：`columnVisibility` / `onColumnVisibilityChange`（D0-7）· `enableRowSelection` / `rowSelection` / `onRowSelectionChange`（D0-3）⇒ 副作用 = 装官方 `components/ui/shadcn/checkbox.tsx`（design §13 F-5）
...
| **T11-j**（表格族统一 · **门户笔**：统一件 + 门户 preset + 排序档收敛 + 3 条部分索引） | 📋 **已立案（未动工）** | — | 依据 = 跨批 design **v1.9**（8 维 **9.31** · 无待拍项）· 步序 **j1→j5** · 含 **1 次迁移**（`0013_*`）· 零新依赖 · 新建件 1（官方 `checkbox`）|
