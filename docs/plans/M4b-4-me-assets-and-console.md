...
> Date: 2026-09-18
> Updated: 2026-09-21（**v0.48：`T11-k` `k2` 落地** —— ① §3 增 `k2` 落地记录（两页 `rowActionsHeader` · `m4b3` 新增 `G16`–`G19` · 首跑 4 FAIL 逐条真因 ⇒ 复跑 **43 PASS / 0 FAIL**（39 → 43 = +4））② §7.6 登记 **F126**（`CheckboxItem` keep-open / `MenuItem` 关闭 的差异会污染断言链）③ 依据 = 批 design **v1.18**。**头部补登记**：头部只留最近 3 条 ⇒ 同笔下沉最旧一条 v0.45（完整历史见 §9 修订记录））
> Updated: 2026-09-21（**v0.47：`T11-k` 控制台笔 `k1` 落地（含两次用户追加拍板）** —— ① `k1` 主体（排序头三列 · 列开关 9 项/保护 2 · 名称列色块 20px 未撑高行 · `api/me.ts` 契约成对登记）② 追加 A **视图切换**（默认列表 · 不记忆 · 复用 `AssetGrid`/`AssetCard` + `status` 槽；**`AssetCard` 未迁 `ui/`** —— 依赖 `console/asset-stats` ⇒ 按跨层引用，偏差已申报）③ 追加 B **搜索对齐门户**（无框钮 + 撑满折叠面板 + 自动聚焦）④ **D0-5 订正**（列表态只列头 / 卡片态用门户排序图标钮）⑤ i18n **+0 键**（7 键跨组复用）⑥ 实测 `k1` **11/0** · 搜索 **5/0** · 视图 **6/0** ⑦ **F124**（`j6` 角标：文档声称已落而码未落 ⇒ 本轮补齐）· **F125**（越界 `page` ⇒ 页面级空态 · 预存 · 「回第 1 页」按同源记）⑧ 路由真值 **`/dashboard/assets`** ⑨ 依据 = 批 design **v1.18**））
> Updated: 2026-09-21（**v0.46：`j6` 追加笔（工具条去框化 + 名称列色块）落地** —— 用户 2026-09-21 两条拍板（「列显示不要外框（同搜索）· 排序简化成图标、也没有外框」⇒ 线框三方向取**方向 1「纯图标」**；「资产名称列除了文字，可以像卡片一样加一下图标么」⇒ 色块 **24px** · 列宽不动）。① **排序控件换案**：官方 `Select`（**160×32**）→ **纯图标钮（32×32）+ 官方 `DropdownMenu` 三档 radio** ⇒ 工具条**省 128px**；`query.setSort(next)` 签名**一字未改** ⇒ **排序语义零变化**（手动路径 URL 只写 `?sort=` · 方向仍归列头两态）② **新增 2 件**：`market/ColumnVisibilityMenu.tsx` **95 行** · `market/SortMenu.tsx` **79 行**；统一件 `meta.hidable === false` 落 `enableHiding: false` ⇒ **保护列硬保证**③ **名称列 +`AssetAvatar` 24px**（与卡片**同源件** ⇒ 实测**同资产同色**）④ i18n **+3 键**（`colShow`/`colRequired`/`colReset`）⇒ **350 键**（zh ⇄ en 同数）⑤ 断言：`m4b4` **89/0**（`G22-1/3/5/6c/8` 五条 + `G21-3` 容器上移一层 + 新增 `closeMenu` 助手）· `m4a` **60/0** 零回归 ⑥ 运行期探针 **8/0**（去框）· **5/0**（色块）⑦ **F121**（`PORTAL_COLUMNS` 漏「操作」槽列 ⇒ 菜单 6 项 ≠ 拍板七项）· **F122**（控件群容器多一层 ⇒ `G21-3` 工具条宽度探针抓内层）· **F123**（探针模板串内注释带反引号致字符串截断 —— **F70 同族复发**）⑧ 代码 18 维 **9.47** ⑨ 依据 = 批 design **v1.17**（8 维 **9.40**））
> 更早版本（**v0.1–v0.42**）摘要见 **§9 修订记录** —— 头部只留最近 3 条（2026-09-21 头部卫生 · 用户拍板）
> Status: **✅ 全绿（2026-09-18）**（**T1–T16** 全绿 · **T8 作废**；T11 收尾已完成：造数 + G1–G19 **53 PASS / 0 FAIL** + 五门禁 exit 0 + 证据回填；**v0.20 验收期第二笔 T11-e 已实现**（G20/G21 新增 ⇒ **77 PASS / 0 FAIL**）· 文档回写完成 · **已提交推送**（`39d1b21` · `9219069`））；**验收期第三笔 T11-f（资产排序）2026-09-20 全绿落地**（`f1 9.35 · f2 9.46 · f3 9.46 · f4 9.47` · 服务端 `sort`/`dir` 契约变更 · 零迁移）· **T11-g（验证效率）全绿**（代码 18 维 `9.49`）· **验收期第六笔 T11-j（表格族统一 · 门户笔）执行中**（`j1` ✅ 已提交 `ce969cb` · `j2` ✅ `0b2d2f2`（门户 preset **286 → 172** · 代码 18 维 **9.48** · 探针 **10/0**）· `j3` ✅ `58ff23e`（档收敛 **5 → 3** · **净零 347 键**）· `j4` ✅（+3 部分索引 · **6 条**索引实证 · 迁移 **`0013_`** · 20 万行实测 ≈265–433×）· `j5` ✅（`m4b4` **89/0** · `m4a` **60/0** · `m4b3` **39/0**）· **`j6` 追加笔** ✅（工具条去框化 + 名称列色块 · 新增 2 件 · **350 键** · 代码 18 维 **9.47** · 未提交）⇒ **门户笔（T11-j）全绿收口**；**下一笔 = `T11-k` 控制台笔**）· **三笔已提交推送**（`72e48db` / `434043b` / `ced0d6a`））；其中 **T8 作废**；**执行序 = T15 最先** —— star 能力先行，再消费面）—— 上游批 design 已**定稿**（8 维**以批 design 版本头为准**（现行 **9.69**）；v1.7 原型评审收口）
...
**T11-j / T11-k（表格族统一 · 两笔）步序 = `j1 → j2 → j3 → j4 → j5 → j6`（门户笔 · `j6` = 追加笔）⇒ 验证绿 ⇒ `k1 → k2`（控制台笔）**
...
**j4 断言 / 门禁**：① `\d asset` 实见 **6 条**索引（既有 3 + 新 3）② **v0.44 订正（F117）**：`EXPLAIN` 命中口径**两段式** —— 真库（29 行）规划器**正确地**走 `Seq Scan` ⇒ ① `SET enable_seqscan=off` **反证索引可用**（实测三档 + 联合谓词均走部分索引）② **实际受益**由 20 万行实验证明（原口径「`EXPLAIN` 三档命中」在小表上**不可满足**）③ `count(*)` **不受益**（不误判为收益）④ `db:migrate` 二次执行不产新文件 ⑤ 实验表**零残留**（`pg_class` 计数 0）⑥ **反证分支**：若 `status + type` 前缀实测**不命中** ⇒ 只做 **design §1.6 登记**（**不临时改列序、不加第 4 条索引**），实测数字照样回填 —— **实测命中**（+6.290 → 0.016 ms）⇒ 走正路

**落地记录（2026-09-21 · `j4`）**：**件** = `db/schema/assets.ts` **142 → 161**（+3 部分索引定义 + 块注释；`import { sql }` 补入）· `drizzle/0013_good_wolverine.sql`（**drizzle-kit 生成** · 文件内**恰 3 条 `CREATE INDEX`** · 无杂项漂移）· `drizzle/meta/*`（生成物 · biome 已排除）。
**实测**：`\d asset` = **6 条**索引（`asset_pkey` / `uq_asset_slug` / `idx_asset_status` + **`idx_asset_newest` / `idx_asset_downloads` / `idx_asset_stars`**，谓词均 `WHERE status = 'ACTIVE'::text`）；`db:migrate` 两次应用**幂等**（末号仍 `0013`）；**实验表残留 0**。
**迁移前对照实测（20 万行 · 独立表 `exp_asset` · `INSERT 0 200000` 自证 · 跑完 DROP）**：
| 查询（LIMIT 20） | 基线 | 加索引后 | 结论 |
|---|---|---|---|
| `newest` | 8.666 ms | **0.020 ms** | ≈433× |
| `downloads` | 7.944 ms | **0.030 ms** | ≈265× |
| `stars` | 9.013 ms | **0.030 ms** | ≈300× |
| **B5 `status AND type`**（中心页谓词） | 6.290 ms | **0.016 ms**（`Index Scan using exp_newest`） | **前缀命中 ✓**（F-6 闭合）|
| **B3 owner + `status=ALL`**（控制台默认态） | 4.550 ms | **4.666 ms** | **不受益**（与登记口径一致）|
| `count(*)` | 5.234 ms | **5.192 ms** | **不受益** |
| B2 参数化 `status=$1`（第 9 次） | — | **0.018–0.036 ms** | **命中 ✓** |
建三条索引：**23.762 / 33.459 / 35.849 ms** ⇒ 合计 **93.1 ms**（与 v1.9 的 ≈0.13 s 同量级 ⇒ 锁窗口口径取保守值 **≈0.13 s**）。
**纪律沉淀（F116/F117/F118）**：① **实验必须先自证数据入表**（首跑 `integer out of range` ⇒ 表空 ⇒ 0.0x ms 假象）② **小表 `Seq Scan` 是规划器自由选择**，不是「索引不可用」⇒ 断言改「关 seqscan 反证可用 + 20 万行证受益」③ **档位收敛的活口径要扫到「档名」一级**（`name` / `author`）—— 只扫「五档」字样会漏（F118 即 §1.6 现状行）。
...
**落地记录（2026-09-21 · `j5` · **门户笔收口**）**：**断言改写**（`m4b4-personal-b-dogfood.ts` **1528 → 1546**）= `G20-3` 真值口径（**恰 1 个进详情锚点 / 0 个 `button`** —— `Button asChild` 渲染为 `<a>`）· `G20-4` **6 → 7 格**（列头加「操作」· 名称列改**纯文本**断言 · 链接断言挪 `tds[6]`）· **新增 `G20-10`**（点操作钮 ⇒ 真跳 `/assets/:slug`）· `G22-3` **四档 → 两档** · **`G22-6` 整条删除**（同列两态由 `G22-6c` 承载）· `G22-7` **五列 → 三列** · **`G6` 锚点改本批前缀**（**F119**）。
**首跑 = 85 PASS / 4 FAIL**（4 红全为**本设计未预见项**，如实留痕）：**F119**（`G6` 按全表行数 ⇒ 被 `m4b3` 跨批夹具污染，真值 5）· **F120**（**删 `G22-6` 的 setup 副作用** —— 那一条顺带做了「切列表视图」前置，删除后 `G22-6b`/`G22-7`/`G22-8` 三条**连带假红**）。
**修完复跑（全量 · 先 seed 复位）**：**`m4b4` 89 PASS / 0 FAIL / 0 CDP 超时 / NO JS ERRORS**（**89 = 91 − 1（删 G22-6）+ 1（新增 G20-10）− 2（G22-3 四档→两档）** 逐项对上）· **`m4a-dogfood` 60 PASS / 0 FAIL / exit 0**（与 T11-i 基线 60/0 逐项一致 ⇒ **零回归**）· `m4b3` **39/0**（`j1` 轮）。
**门禁** = **CI 同序八步全绿**：`typecheck` / `lint` / `format:check`（276 文件）/ `doc-audit` **66 PASS** / `build` / `db:migrate` / `CI=true test` **559 pass · 1 skip · 0 fail**（`protocol` 另 27 例）。
**证据**：`docs/smoke/2026-09-18-m4b4-personal-b.md` **→ 611 行**（活口径 5 处订正 + **新增 §14.15 T11-j 块**）· 截图**重拍 11 张**（含列表 7 列两图）· 原 `26-sort-header-desc.png` 不再产出。

---

**j6 · 工具条去框化 + 名称列色块（追加笔 · 2026-09-21）**

**立项**：用户两条拍板 —— ①「**列显示不要外框（和搜索没有外框一样）** · **简化排序成图标的形式，也没有外框**」⇒ 出**线框三方向**对比（`/tmp/aih-table-alignment/j6-styleboard-v2.html` · 三张实拍图）后取 **方向 1「纯图标」**；②「**资产名称列除了文字，可以像卡片一样加一下图标么**」⇒ 按推荐 = **色块 24px · 列宽不动**（控制台名称列归 `T11-k`）。

**契约与实现**：① 排序控件换案 = 官方 `Select`（**160×32**）→ **纯图标钮（32×32 · `ghost`）+ 官方 `DropdownMenu`（`RadioGroup` 三档 · 当前档 tooltip）** —— 换案依据 = **选型矩阵已登记案**（不是新增取舍）⇒ 代价「当前档不可见」由 tooltip + 菜单勾选补足；`query.setSort(next)` **调用签名一字未改** ⇒ 方向语义与旧 `Select` 逐项一致（`G22-3` 断言钉死）② 列显示钮 `outline` → `ghost`（去框）+ **角标承担「有值」**（藏列数）③ 名称格 = `[AssetAvatar 24px] + 文本（`truncate` + `min-w-0` 必需）`，复用卡片同源件 ⇒ **同资产同色**④ 统一件补口：`meta.hidable === false` ⇒ `enableHiding: false`（保护列**硬保证**；动作槽列本就在 `columns` 之外）⑤ 两页工具条：右侧控件群包一层 `ml-auto` **容器** ⇒ 组内条件渲染（列显示仅列表视图）时**右边缘恒定**。

**断言 / 门禁**：`m4b4` **89 PASS / 0 FAIL**（脚本 **1546 → 1585**）· `m4a` **60/0** 零回归（**979**）· 运行期探针 **8/0**（三钮 `border 0px` · 各 32×32 · 排序钮无文案 · 群右边缘恒定 · 网格态列显示不渲染）+ **5/0**（色块 24×24 · `aria-hidden` · **与卡片同色** · 名称格不溢列宽 · 卡片仍 40px）· 页内快速门禁（typecheck / format / lint）全绿。

**新增发现**：**F121** —— `PORTAL_COLUMNS` 初版只列 6 个列定义项、**漏「操作」槽列** ⇒ 菜单与拍板 M-A（**七项全列**）不符；槽列无列 id ⇒ 天然不可隐藏（补入后 **7 项 · 保护列 2 项置灰**）。**F122** —— 控件群多包一层容器后，`G21-3` 的「工具条宽度」探针按 `searchBtn.parentElement` 取到**内层容器**（真值 171 ≠ 1129）⇒ 该类探针须**按结构层级上移一层**（组内坐标不变，只有宽度量错）。**F123** —— 探针模板串内的注释写了**反引号** ⇒ 模板串被截断、整文件 parse 错（**与 F70 同族复发**）⇒ 纪律再确认：**脚本内注释一律不写反引号**。
...
**主体（plan 原定 3 条）**：① `pages/Assets.tsx` 加 `sort` 维度（`useMarketQuery({ status, sort })`）+ 列定义三列携 `meta.sortKey`（下载/收藏/更新 → `newest`）· 回第 1 页由 hook 内置 `dropPage` 承担（**Q2 无需页面实现**）② `api/me.ts` **参数面 + 契约行成对登记**（`sort`/`dir`）③ 列开关入口 = **工具条**（跟齐门户 · 见式样追加）· 菜单 **9 项** · 保护列 **2**（名称/操作）置灰「必显」· 不持久化 ④ 名称列 + `AssetAvatar` **20px**（控制台行高 40 ⇒ 比门户小一档）· **A/B 实测未撑高行**（临时隐藏 ⇒ 53/53）

**追加 A · 视图切换（用户 2026-09-21「能也支持卡片和列表切换显示么」⇒ 按推荐）**：**默认列表** · 不记忆 · 复用门户 `AssetGrid`/`AssetCard`；**`AssetCard` 新增可选 `status?: ReactNode` 槽**（门户不传 ⇒ 零变化）；**件未迁 `ui/`**（依赖 `console/asset-stats` ⇒ 归位会造 `ui/`→`console/` 反向依赖 ⇒ 按跨层引用处理 —— **与上报口径的偏差，如实申报**）；**D0-5 订正** = 列表态只列头可点、**卡片态加门户同款排序图标钮**（两态各一套入口）

**追加 B · 搜索对齐门户（用户 2026-09-21「搜索也对齐一下门户的搜索」）**：常驻 240px 有框 `Input` ⇒ 无框 `Search` 图标钮 + 工具条下方撑满折叠面板（官方 `InputGroup`）+ **展开自动聚焦**；搜索页无需改（真码核实无工具条搜索控件）

**门禁 / 真值**：`k1` 探针 **11 PASS / 0 FAIL** · 搜索对齐 **5/0** · 视图切换 **6/0** · 页内快速门禁全绿 · **i18n +0 键**（7 键跨组复用 ⇒ **350**）· 真图 3 张（仓外）

**新增发现**：**F124** —— `j6` 设计/计划声称「列显示钮**角标**承担已藏列数」，而**已提交代码里没有角标**（探针亦未覆盖）⇒ 本轮补齐（`ColumnVisibilityMenu` 加 `badgeCount` + 两页传值）+ 登记「文档声称 / 代码未落」。**F125** —— URL 带**越界** `page` ⇒ 渲染**页面级空态**（文案「从未有资产」，`thead` 不渲染）· **预存行为** ⇒ ① 语义偏差（应表内空行/回第 1 页）② 使「从第 2 页改排序回第 1 页」在控制台**端到端不可达** ⇒ 该断言按**同源**记（`dropPage` 在 hook · 门户 `G22-4` 已覆盖）。

**路由真值订正**：「我的资产」= **`/dashboard/assets`**（`main.tsx:108`）—— 本会话曾口头误报 `/console/assets`。
...
**范围边界（用户 2026-09-21 拍板）**：「我的提交」/「我的令牌」**只做表头统一（`k2`）**——**不做**列开关 / 视图切换 / 排序对标。理由（用户原话）：「**我的提交主要看状态**」。另：`/api/reviews/mine` 真值 = 已按 `submittedAt DESC, id DESC` **硬编码「最新」序**、**无 `sort`/`dir` 参数**、item **无「名称/描述」字段** ⇒ 排序与卡片视图**须先改服务端契约**（本次不做 · 若将来要做按「契约变更先落 design/plan」起独立小笔）。

**k2 落地记录（2026-09-21）**：① `Submissions.tsx:240` / `Tokens.tsx:303` 各 **+1 `rowActionsHeader`**（复用 `submissions.col.actions` / `tokens.col.actions` ⇒ **零新增键**；`Assets.tsx:244` 本就有 ⇒ **三页可见「操作」表头齐**）② `m4b3-personal-a-dogfood.ts`（**891 → 1049 行**）新增 **`G16`–`G19`** 四条：`G16` 表头 9 列 + 「操作」可见 + **仅**下载/收藏/更新 可点 · `G17` 列头两态 + URL 同步 + **序与接口逐项一致**（走接口带会话 cookie · 只用本账号数据）· `G18` 列开关 9 项/保护 2 置灰「必显」/全 checked/角标 1/重置回 9/保护列关不掉 · `G19` 视图切换（默认列表 · 两钮互斥 · 卡片 40px 色块 + 状态徽标）+ 搜索对齐（面板撑满 1081px + 自动聚焦）③ **首跑 4 FAIL 如实留痕**：`G2`×2 / `G3` = **未重 seed**（PENDING 行被上一跑消耗 —— 仓内已登记坑）；`G19` = 前置动作写错（`DropdownMenuItem`「重置为默认」**会关闭菜单**，我又点了一次触发钮 ⇒ 菜单复开、后续点击被遮罩吃掉）⇒ 修完复跑 **43 PASS / 0 FAIL**（**39 → 43 = +4** 与新断言数逐项对上）④ 控制台截图重拍（含新图 `m4b3-05-console-grid.png`）⑤ **F126** 登记

**k2 断言 / 门禁**：① 两页表头**可见「操作」**（`sr-only` 兜底不再生效）② `G16`–`G19` **全绿**（实测 **43/0**）③ `m4b3` 既有断言**零回归**（39 条全绿 + 4 新条）④ 五门禁 + `doc-audit` exit 0 / 全 PASS
...
> 本栏 = **执行期回填**（2026-09-20 起逐条追加；格式对齐 M4b-1 / M4b-3 先例）—— 原导语「待执行」于 2026-09-24 跨批体检修正。
...
| 出口五件 | ① design 8 维 **9.69** ✅ ② **T1–T16 全绿** ✅ ③ 五门禁 exit 0 ✅ ④ dogfood **现行 G1–G22 = 91 PASS / 0 FAIL** + 门户 **38/38** ✅（T11 时点 53/0 · 36/36）· 观感：T11-d 三连已获认可；**T11-e 三项待拍板 ⬜**（hover 口径 / 表头底纹 / 行高档位 —— 表头底纹已按建议落地、待验）〔2026-09-25 体检：**本条为 T11-e 时点快照**；其后 T11-f…j 轮次的定案见本文档 §7.5/§7.6 与跨批 design `2026-09-21-table-family-alignment-design.md`（表格族统一轮 · 控制台表格密度 = 40）〕⑤ 整体审计**无未决项** ✅ |
...
| **T11-j**（表格族统一 · **门户笔**：统一件 + 门户 preset + 排序档收敛 + 3 条部分索引） | ✅ **全绿（门户笔收口 · 2026-09-21）** | `j1` **9.34** · `j2` **9.48** · `j3` **9.47** · `j4` **9.53** · `j5` **9.49** · **`j6` 9.47** | 依据 = 跨批 design **v1.16**（8 维 **9.38** · 门户笔设计侧收口）· 步序 **j1→j5** · 含 **1 次迁移**（`0013_*`）· 零新依赖 · 新建件 1（官方 `checkbox` · **已装 26 行**）· **`j1` 门禁** = 四门禁 **exit 0** + 探针 **30/0** + dogfood 对照（干净 HEAD **38/1** ⇄ 带 `j1` **38/1**）**零回归** ⇒ 已提交 `ce969cb`｜**`j2` 门禁** = 四门禁 **exit 0** + 运行期探针 **10/0** ⇒ 已提交 `0b2d2f2`｜**`j3` 门禁** = CI 同序七步全绿（`test` **559 pass / 0 fail**）⇒ 已提交 `58ff23e`｜**`j4` 门禁** = `\d asset` **6 条** · 二次 migrate 幂等 · 实验**残留 0** · 迁移前对照实测（20 万行 7 行数字）⇒ 已提交 `583d2aa`｜**`j5` 门禁** = `m4b4` **89/0** · `m4a` **60/0** · `m4b3` **39/0** · CI 同序八步全绿 · 证据回填完成 ｜**`j6` 门禁** = 新增 2 件（**95 / 79 行**）· `m4b4` **89/0** · `m4a` **60/0** · 探针 **13/0** · i18n **350 键** · 三件快速门禁绿 ⇒ **已提交推送 `af8c134`**（CI 全绿） ⇒ **门户笔全绿**（下一步 = `T11-k` 控制台笔 · 独立一笔）|
| **T11-k**（表格族统一 · **控制台笔**：「我的资产」排序 + 列开关 + 视图切换 + 搜索对齐 + 存量两页表头） | ✅ **全绿（2026-09-21）** | `k1` **9.44** · `k2` 43/0 | **前置 = T11-j 全绿** ✅ · 步序 `k1`→`k2`（`k1` 含两次用户追加：**视图切换** / **搜索对齐门户**）· 新增断言 **G16–G19** · **D0-5 口径订正**（列表态只列头可点 / 卡片态用门户同款排序图标钮）· 新增 2 件（`market/SortMenu` · `ui/ColumnVisibilityMenu`〔自 `market/` 迁入〕）· 提交推送 **`b30607e`** · CI 全绿 |
...
- **T11-j `j4` 落地（v0.44 · 2026-09-21）**：**F116** —— **实验读数前必须自证「数据真进了表」**：首跑 `INSERT` 因 `(g * 104729)` **int4 溢出**失败（`integer out of range`），而 psql 默认**继续执行后续语句** ⇒ 表空、全部 EXPLAIN 给出 **0.0x ms + `actual rows=0`** 的**假象**（差一步就写进文档）。处置：加 `\set ON_ERROR_STOP on` + `g::bigint` 转换；**读任何数字前先看 `INSERT 0 200000` 与 `actual rows > 0`**。**F117** —— **断言口径在小表上不可满足**：原写「`EXPLAIN` 三档查询命中部分索引」，而真库仅 **29 行** ⇒ 规划器**正确地**选 `Seq Scan`（0.044–0.053 ms）。订正为**两段式**：`SET enable_seqscan=off` **反证可用** + 20 万行证**受益**。**F118** —— **`j3` 连带陈旧引用**：§1.6「现状无感」行仍引 `name` 档 0.139 ms（该档已下线）；`j3` 的活口径扫描只扫了「五档」字样、**扫不到只写档名的引用** ⇒ 沉淀：**档位收敛要连档名一起扫**。

- **T11-j `j5` 落地（v0.45 · 2026-09-21）**：**F119** —— **跨批夹具污染**：`m4b4` 的 `G6` 按**全表行数**断言「三态各一行 = 3」，而 `m4b2_user` 名下另有 `m4b3` 批的 2 条 ACTIVE 资产（`m4b3-seed-skill` / `m4b3-seed-mcp`）⇒ 真值 **5** ⇒ 假红。改**本批前缀锚定**（`ownRows` 按行内操作列 `href` 前缀 `/assets/m4b4-seed-` 计数）⇒ 与 AGENTS.md「**测试只依赖自己造的数据**」一致。**F120** —— **删断言的 setup 副作用**：`G22-6` 整条删除后，`G22-6b` / `G22-7` / `G22-8` **连带假红** —— 被删的那条**顺带做了「切列表视图」前置**（无 `thead` 则列头断言全空）。处置：把前置**显式补进 `G22-6b`**。**纪律沉淀**：**删一条断言前必须盘它的「前置动作」与「共享状态」** —— 断言往往不只是断言，还是后续断言的 setup。

- **T11-k `k1` 落地（v0.47 · 2026-09-21）**：**F124** —— **文档声称与代码不符（自产）**：`j6` 的设计 §3.4 ② 与 plan 契约项都写了列显示钮以「**角标**承担已藏列数」，但提交的 `ColumnVisibilityMenu` **没有角标实现**、j6 探针 8 条也没覆盖它 ⇒ 本轮补齐（`badgeCount` + 两页传值）并登记。**纪律**：**「可选能力」写进文档时必须同时落码或标为未实装** —— 否则 audit 查不出（`doc-audit` 不查代码）。
- **T11-k `k1` 落地（v0.47 · 同日）**：**F125** —— **越界 `page` 的页面级空态**：`/dashboard/assets?page=2`（库内仅 1 页）渲染 `Empty`（`thead` = 0）且文案为「**从未有资产**」⇒ ① 语义偏差（应「筛选无结果」或回第 1 页）② **连带影响断言**：控制台无法端到端复现「改排序回第 1 页」⇒ 该机制改按**同源**记（`dropPage` 在 hook 内 · 控制台与门户**共用同一函数** · 门户 `G22-4` 覆盖）。**纪律**：**写断言前先确认前置状态可达**（「先翻到第 2 页」这类前置在 1 页数据下不可达 ⇒ 探针会以「表头为 0」的形态连锁全红）。
- **T11-k `k2` 落地（v0.48 · 2026-09-21）**：**F126** —— **菜单生命周期混用会静默吃掉点击**：官方 `DropdownMenuCheckboxItem`（列开关用）勾选**不关菜单**（`onSelect` preventDefault），而 `DropdownMenuItem`（「重置为默认」）**点击即关闭**；断言链里混用两类项时，**多点的每次「触发钮」都会反转开合** ⇒ 随后点击落到遮罩上失效（`G19` 首跑实测：网格切换静默不生效、页面仍读成列表态）。**纪律**：**点触发钮前先确认菜单开合**（`aria-expanded` / `data-state`），或切换视图前显式关菜单。

**纪律沉淀**：**断言条数类数字必须带「组成式」** —— 本轮写「运行期探针 **35 条全绿**」，但读者无法核对：35 = 去框 8 + 色块 5 + `k1` 11 + 搜索 5 + 视图 6，且**不含** `j6` 首轮 12 条 ⇒ 真实全量 **47**。**F128** = 自产数字**无组成式** ⇒ 无法反查（与 F28「13 表 vs 实测 12 表」同族）。**纪律**：凡「N 条断言 / N 个用例」类声明**必写各支脚本各贡献几条**。

**纪律沉淀**：**「可见性」类改动不能用 `textContent` 断言兜底** —— `sr-only` 文本**同样**出现在 `textContent` 里 ⇒ 既有表头断言（`m4b3:369` · `:523` 以 `textContent` 取列头数组）在「`sr-only` 兜底」与「可见表头」**两种实现下都通过**。**F127** = 本笔 `rowActionsHeader`（`sr-only` → **可见**）的**可见性属代码级确认、无专属断言**（待补：断言 `th` 内无 `.sr-only`）。**断言要落在可见性载体上**（类名 / 尺寸 / `computedStyle`）。

- **T11-j `j6` 落地（v0.46 · 2026-09-21）**：**F121** —— **单一源清单漏项**：`PORTAL_COLUMNS` 作为「列集合单一源」初版只枚举了 6 个**列定义项**，**漏掉「操作」槽列**（槽列不在 `columns` 内，却在用户拍板的 M-A 菜单里要**显示且置灰**）⇒ 菜单只 6 项、与拍板不符（首跑探针当场红）。**纪律**：**清单类单一源必须按「用户可见集合」而非「代码结构集合」枚举**（槽列 / 派生列最易漏）。**F122** —— **结构性探针的层级假设**：控件群多包一层 `ml-auto` 容器后，`G21-3` 按 `searchBtn.parentElement` 取「工具条」⇒ 抓到**内层容器**（`barW` 171 ≠ 1129）⇒ 假红。**纪律**：探针取容器一律**显式声明结构层级**（`parentElement.parentElement` + 注释写明为何），结构改造后**先跑相关段**而非全量。**F123** —— **探针模板串内注释带反引号 ⇒ 字符串截断**（`TS 1005` 解析错）—— **与 F70 同族复发**（本轮第二次踩）⇒ 纪律升级：**写探针时段级注释用文字描述（如「列名 span」），禁一切反引号 / `*/` 字面量**。

## 9. 修订记录

| **v0.48** | 2026-09-21 | sunxuewen-rush | **`T11-k` `k2` 落地** ① §3 增 `k2` 落地记录（两页 `rowActionsHeader` + `m4b3` 新增 `G16`–`G19` + 首跑 4 FAIL 逐条真因 + 复跑 **43 PASS / 0 FAIL**（39 → 43 = +4））② §7.6 登记 **F126**（`CheckboxItem` keep-open / `MenuItem` 关闭 的差异会污染断言链）③ 头部 +本行 ④ 依据 = 批 design **v1.18**。**实测** = `m4b3` **43/0** · 截图重拍（含 `m4b3-05-console-grid.png`） |
| **v0.47** | 2026-09-21 | sunxuewen-rush | **`T11-k` `k1` 落地（含两次追加拍板）** ① §3 增 `k1` 落地记录（主体 4 条 + 追加 A 视图切换 + 追加 B 搜索对齐 + 门禁真值 + 偏差申报）② §7.6 登记 **F124**（文档声称角标而代码未落 ⇒ 本轮补齐）· **F125**（越界 `page` 页面级空态 ⇒ 断言前置不可达 ⇒ 回第 1 页按同源记）③ 头部 +本行 ④ 路由真值订正 `/dashboard/assets` ⑤ 依据 = 批 design **v1.18**（8 维 **9.35**）。**实测** = `k1` **11/0** · 搜索 **5/0** · 视图 **6/0** · i18n **350 键**不变 |
| **v0.46** | 2026-09-21 | sunxuewen-rush | **`j6` 追加笔（工具条去框化 + 名称列色块）落地** ① §3 增 `j6` 子块（立项两条拍板 / 契约与实现 5 项 / 断言门禁真值 / 三条新发现）② §7.6 登记 **F121**（单一源清单漏「操作」槽列 ⇒ 菜单 6 项 ≠ 拍板七项）· **F122**（控件群容器多一层 ⇒ `G21-3` 探针抓内层 · 结构层级须显式）· **F123**（探针模板串内反引号致截断 —— **F70 同族复发**）③ §7.5 `T11-j` 均分补 **`j6` 9.47** + 门禁列补 `j6` 实况（**未提交**）④ §2 文件列补两枚新件 · 步序 `j1 → … → j6` ⑤ 头部 +本行 ⑥ 依据 = 批 design **v1.17**（8 维 **9.40**）。**实测** = `m4b4` **89/0** · `m4a` **60/0** · 探针 **8/0** + **5/0** · i18n **350 键** · 新增 2 件（95 / 79 行） |
| **v0.45** | 2026-09-21 | sunxuewen-rush | **`j5` 门户笔收口落地（零行为决策 · 仅登记）** ① §3 `j5` 补**落地记录**（断言改写 1528 → 1546 · 首跑 85/4 如实留痕 · 修完复跑 **89/0** · `m4a` **60/0** · 证据 611 行 + 11 图）② §7.6 登记 **F119**（`m4b4` `G6` 跨批夹具污染 ⇒ 改本批前缀锚定）· **F120**（删 `G22-6` 的 setup 副作用 ⇒ 前置显式补入 `G22-6b`）③ §7.5 `T11-j` → **✅ 全绿**（`j5` 代码 **9.49**）④ 头部 +本行 ⑤ 依据 = 批 design **v1.16**（8 维 **9.38**）。**实测** = 三套 dogfood（89/0 · 60/0 · 39/0）+ CI 同序八步全绿 |
| **v0.44** | 2026-09-21 | sunxuewen-rush | **`j4` 索引迁移落地回填（零行为决策 · 仅登记）** ① §3 `j4` 补**落地记录**（schema 142 → 161 · `0013_good_wolverine.sql` 恰 3 条语句 · `\d asset` **6 条** · 二次 migrate 幂等 · 残留 0）+ **迁移前对照实测表**（20 万行 7 行数字：三档 ≈265–433× · B5 命中 · B3/计数不受益 · 建索引 93.1 ms）② §3 `j4` 断言② 同批订正（**F117** 两段式口径）③ §7.6 登记 **F116**（实验须自证数据入表）· **F117**（小表 `Seq Scan` 是自由选择）· **F118**（`j3` 连带陈旧引用 —— 档名级扫描）④ §7.5 `T11-j` 行 → `j4` ✅（代码 **9.53**）⑤ 头部 +本行 ⑥ 依据 = 批 design **v1.15**（8 维 **9.35**）。**实测** = DB 侧全绿 · 无服务端代码变更（仅 schema 索引定义 + 迁移文件）|
