...
> Date: 2026-09-11
> Updated: 2026-09-17（**v0.26：首页 hero 三处调整登记**（用户 2026-09-17 拍板）—— ① 整个 hero 去卡（`Card` → `div`，内容落底）② 搜索行改官方 `InputGroup` 框内形态（= M4b-1 §6.2 **D′ 项收口**）③ 胶囊化 `rounded-full` + `h-11`（44px）+ 单放大镜（右侧提交钮））
...
- **③ 冒烟**：`m4a-chain-smoke.ts` ✅ **29 断言全 PASS** + `CHAIN SMOKE PASS`（二轮审计按实测修正：脚本 `ok(` 调用点 = 29；原「30/30」把汇总行计入）· `m4a-dogfood.ts` ✅
...
| 16 | **3 处文档仍引用已删旧层文件**（T24 换靶发现） | ① `2026-09-09-m4a-marketplace-portal-design.md:190` 目录树行 `└── styles/  tokens.css · global.css`（T24 后该目录只剩 `aih-theme.css`）② `2026-09-10-m4b-admin-console-and-auth-design.md:29`「`tokens.css` 全量视觉 token 可直用」③ 同文档 `:670` 复用清单含 `styles/tokens.css` | ✅ **已处置（T26）**：三处（design §4.2 目录树 · m4b design L29/L670）一律改指 **`styles/aih-theme.css`** |
...
| 25 | **9 个 token 零消费者**（二轮审计发现·utility 感知扫描） | `aih-theme.css` 定义 65 个 token，真零消费者 9 个：① `--color-ava-2..8`（7 个别名——原值 `--ava-*` 已被 AssetAvatar 的 `var(--ava-${i})` 消费，别名无人用）② `--color-success-foreground` / `--color-warning-foreground`（design §4.4 ② **明文列的配对值**，当前无实底 success/warning 徽章故未消费） | ① 别名 7 个：可删（写法改 `--ava-*`）或保留（便于 `bg-ava-N` utility）——**待拍板**；② foreground 两项**保留**（设计配对完整性，随首个实底成功/警告徽章启用）；**不阻塞** |
| 26 | **冒烟记录缺分辨率矩阵**（二轮审计发现，**本轮已补实测**） | stage-0 记录有 13 档分辨率矩阵，stage-1 记录（`s1`）只有 1440 单档 ⇒ 本轮补 **10 档**实测：1920 / 1440 / 1366 / 1280 / 1200 / 1100 / 900 / 880 / 850 / 760——全程**零横向溢出**；列数逐档命中 design「<1200→3 / <900→2」（1280→4 · 1100→3 · 880/850/760→2）；侧栏各档 expanded 204px（<768 才切移动端 Sheet） | 本轮已补证据（见审计报告）；建议：**下个里程碑的冒烟记录直接复用 stage-0 的 13 档矩阵做法**（把分辨率矩阵列为记录必备节）——**待拍板** |
...
| v0.26 | 2026-09-17 | sunxuewen-rush | 首页 hero 三处调整登记（用户 2026-09-17 拍板）**—— ① 整个 hero 去卡（`Card` → `div`，内容落底）② 搜索行改官方 `InputGroup` 框内形态（= M4b-1 §6.2 **D′ 项收口**）③ 胶囊化 `rounded-full` + `h-11`（44px）+ **单放大镜落右侧**当提交钮 ④ 底色保持透明 ⑤ 依据 = M4a 门户 design **v0.29** §8.9（实测 44px / 框内 svg=1 / addon=1 / 按钮 32×32 / hero 842 · dogfood **36/0** · 门禁 exit 0）；**v0.25：二轮审计修正（N1/N2）+ 登记（25-26）**——`m4a-chain-smoke` 口径「30/30」→ **29 断言 + 汇总行**（5 处：本文件 + s0 记录 + `00` 三处）· 两处注释腐化（AppShell「未换皮」/ main.tsx「占位页待替换」）已更新 · 新增登记 25-26（零消费者 token 9 个 / 冒烟记录分辨率矩阵做法）；**v0.24：整体审计（T17-T26）修正 + 登记（20-24）**——两处自产数字按实测修正（`.module.css` 12→**16**·§4.4 ⑦ 44→**93 属性/29 token 名**）+ §3 表 20-24 五项登记；审计结论 代码 **9.62** / 文档 **9.53**；**v0.23：阶段 1 完成 + converge（T26）**——T17-T26 全 ✅：详情页 / 中心页换皮（`.module.css` **16 → 0**（16→14→12→9→2→1→0））· design 正文语义回写 18 行 · 删旧层（样式单入口 = `index.css` + `aih-theme.css`）· 五门禁 **475 例 474 pass / 1 skip / 0 fail** · 全态冒烟 **36/36 + NO JS ERRORS** · 观感用户确认 OK · converge 三查 + design **8 维重评 9.56** + `00` §5 M4a 行改 **✅ 完成**；**v0.10：阶段 1 任务细化（T16 出口动作）**——新增 **§2 板块 E/F/G**：**T17-T20 详情页换皮**（头区+Tab / Overview+Markdown / Files+FileTree+预览对话框 / VersionCompare+Diff）· **T21-T22 中心页换皮**（CenterPage / FilterStrip）· **T23-T26 收尾**（17 处 M4-pre 语义回写 / 删旧层 `.module.css`×16 + `tokens.css` + `global.css` / 阶段 1 门禁+全态冒烟 / converge 重评）；§3 补「**附带评估项**」5 条（大屏留白 / `apps/web` 测试基建 / 包体 code-split / tooltip 人工确认 / lint warning 清债）；Status 改「执行中（阶段 1）」 |
| v0.25 | 2026-09-11 | sunxuewen-rush | **二轮整体审计修正 + 登记（25-26）**：① **N1 数字修正（扫类命中第 3 例）**：`m4a-chain-smoke` 记录口径「30/30 PASS」→ **「29 断言全 PASS + 汇总行 `CHAIN SMOKE PASS`」**——实测脚本 `ok(` 调用点 = 29（全为无条件顶层调用，base == HEAD 恒定；实测 PASS 行 = 29）⇒ 原数从未成立（把汇总行计入）；涉及 5 处：s0 记录:71 · 本 plan T15 段 · `00` 版本头 / §5 M4a 行 / v1.17 行 ② **N2 注释腐化 2 处**（阶段 1 后失效）：`AppShell.tsx:19`「中心页 / 详情页（未换皮）」· `main.tsx:14`「占位页随板块 C/D/E 替换为真实页面」——已更新为完成态 ③ **登记 25-26**：零消费者 token 9 个（`--color-ava-2..8` 冗余别名 + 设计配对预留的 `*-foreground` 两项）· 冒烟记录缺分辨率矩阵（本轮补 10 档实测：零横向溢出 + 列数逐档命中 design） ④ **二轮审计结论**：代码 **9.69**（↑ 因补齐验证缺口，非产物变好——如实披露）· 文档 **9.53**（↓ 因 N1/N2 新缺陷抵消轮 1 修复增益）⑤ 版本头升 v0.25 |
