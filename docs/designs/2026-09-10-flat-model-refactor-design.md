...
> Date: 2026-09-10
> Updated: （**v0.5：头部数字修正**——Status「运行库 13 表」→ 实测 **12 表**（同源项 plan v0.6 的 F28 当时只改了 `08` §Status，本文头部漏改；dev 库 `psql \dt` 实测 12 表）；同时补齐头部与 §13 的版本同步（v0.4 此前未进本行**；**v0.4：S3/S4 执行回写 + 规范同步落地**（§12 全部落地 · 清单漏项 F25/F26/F27 · 迁移 0005/0006/0007 实落——详见 §13**）
> **头部口径（2026-09-18 起）**：只留最近 1-2 版 · 不复述历史与验收数字；完整历史见 **13 修订记录**。
> SSOT：实测值与断言数 → `docs/smoke/` 证据文件 · 自检分 → 对应 design 的自检节。
> Status: **定稿 + 已实施完成**（v0.5 表数修正（13→12，实测）· v0.3 · 2026-09-10 用户批准：8 维自检 9.3 ≥9 + grilling Round 1（P1-P7）闭环；v0.3 为 S1 执行偏离回写，**拍板语义不变**；**S1-S4 四板块已于 2026-09-10 全部实施完成**——迁移 0005/0006/0007 实落（运行库 12 表）、四门禁全绿、全链冒烟 24/24 + dogfood 21/21、规范层同步落地（见 §12/§13 v0.4））
...
| 轮次 | 议题 | 结论 |
|------|------|------|
| 对标 | skillhub（双轴 RBAC + 空间自治）· clawhub.ai（**无空间、无可见性、无角色分权**，扁平个人注册中心）| 三系统对比表见 §2.4 |
...
- 对标源：skillhub 源码（Apache-2.0，双轴模型参照）· `https://clawhub.ai/api/v1/openapi.json`（扁平模型参照，2026-09-10 实测）
...
| v0.4 | 2026-09-10 | sunxuewen-rush | **S3/S4 执行回写 + 规范同步落地（T12/T13）**：§2.2 D4 已拍板；§4.1 补 web 同步行（`api/types.ts`/`AssetDetail.tsx`/`i18n`——F21 原边界漏列）；§7 S3 边界补 web 同步与非 ACTIVE 实测口径（**仅 SUPER_ADMIN** 可读——D6 修正）；**§12 规范同步全部落地**（01 §3.3 · 02 §3 · 05 §6 重写（4 档单轴 + owner 语义；小节编号保持）· 06 §1/§3/§5.3/§6 · 08 §3/§4/§5.1/§6/§7/§8 · 00 §2.2/§5（新增 M4-pre 行）/§6/§8 + `packages/protocol/src/slug.ts` 注释）；**清单漏项修复**：F25（05 §1/架构图/§5 三处空间表述未列入）· F26（02 §3 + 06 五处未列入）· F27（AGENTS.md 里程碑状态 stale）；用例账 D5 + 死错误码清理 F24；迁移 0005/0006/0007 实落 |
| — | 2026-09-10 | sunxuewen-rush | **定稿**（用户批准；出口标准：8 维自检 9.3 ≥9 + grilling Round 1 闭环）。实施由 `docs/plans/M4-pre-flat-model-refactor.md` 承接。 |
| v0.5 | 2026-09-10 | sunxuewen-rush | **头部数字修正（文档-代码对齐回查补漏）**：Status「运行库 13 表」→ 实测 **12 表**（dev 库 `psql \dt` 实测 12 行；plan v0.6 的 F28 同源修正当时只落到 `08` §Status，本文头部漏改）；同轮补齐头部 Updated 行与本节版本同步（v0.4 此前未进 Updated 行）。**不改拍板语义**。 |
