...
> Date: 2026-09-23
> Updated: 2026-09-24（**v0.43：① 重挂归属已证（G16）** —— 新增 dogfood **G16**（6 条 · 重挂 ⇒ 看板一级上卷归属）：端点三态对比（原子行消失 / 零计数目标新父 count **严格等值** / 上卷总量守恒 / `rankings` 同面）+ **真页**图例行数对齐 + **复原逐字段回落**（自清）+ **反证**（停重挂 ⇒ 3 条 FAIL）；dogfood 全量 **102 → 108 PASS / 0 FAIL**；顺带**口径纠正**：naive「新父 += 原子」在 `selectDistinct` 去重下不成立（`m4b4-seed-skill` 同时挂 `agentic` 与 `m4b4-seed-privileged` —— `overview.ts` 口径第 6 条已写明）⇒ 断言取零计数目标做严格等值 ② 证据 §8 item 9 由「未证」升「已闭合」）
