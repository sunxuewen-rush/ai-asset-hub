...
> Date: 2026-09-23
> Updated: 2026-09-24（**v0.41：F218 一级可重挂（用户拍板「②」）** —— ① 服务端 `updateLabel` 删「一级不可降级」硬拒 → **安全重挂**（前置 = 目标必须一级 + **自身无子级** ⇒ 400 `label.parent.has_children`；守卫刻意在 `resolveParent` **之后**）② 前端父级下拉**放开**（候选 = 一级且**排除自身**；仅「有子级」禁用 + 说明）③ i18n `parentHint` 改写 + `parentLockedHint` → `parentHasChildrenHint`（**键数不变**）④ **规范层 `docs/06` v1.7** + **明写相对兄弟仓 SkillHub 契约的偏离 + 理由** ⑤ 用例 **F218（5 步）** + dogfood **G8.8/G8.9 改写 + G8.11/G8.12**（真页往返 + 自清）+ 反证 ⇒ **100 → 102 PASS / 0 FAIL**）
> Updated: 2026-09-24（**v0.40：F217 编辑失败不再静默 + F216 已修 + 缺陷登记形态统一** —— ① **F217**：`submitForm`/`confirmDelete` 补 `catch` ⇒ `toast.error(tErr(code))` · 编辑一级标签父级下拉禁用 + 「不可降级」说明 · 补齐全部 `label.*` 错误文案（i18n 527 → **536**）· dogfood **G8.8–G8.10**（含反证）② **F216**：`api/admin.ts` `invalidateLabelCaches()` 三前缀 + 四写函数挂载 · 真页正反双证 · dogfood **G15** ⇒ 断言 **92 → 100**③ 批 design 固定小节 **§9.8** = 明细主家（plan/证据只放指针）· 号规则与跨批总览落 `designs/README` + `docs/README` §6.1）
...
> Status: **定稿**（**2026-09-23 用户批准**）· **实现已落地（T1–T10）· 看板维重做（T6⁺）**—— 文档 8 维见 §11（§11.2 为重做轮复评）· 换靶 5 轮 + grilling 4 轮 · 门槛项已清空 · 已回填 `docs/00` §5（**v1.94**）与主 design §2.3（**v1.69**）；dogfood **102/0**（G1–G15，F218 后）；后续实现以批 plan 为准
...
### 9.8 实施期发现与处置（F203–**F218** · 实现批 T1–T10 + 收口轮）
