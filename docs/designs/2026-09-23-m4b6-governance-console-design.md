...
> Date: 2026-09-23
> Updated: 2026-09-24（**v0.38：F216 登记 + 缺陷登记形态统一** —— ① 用户报缺陷 F216「创建标签后不显示、刷新才出现」根因 = 标签写面漏 `invalidateCache`（语言感知 Promise 缓存未失效 ⇒ tick 重取命中旧 Promise）；② 批 design 固定小节 **§9.8 实施期发现与处置** = 明细主家，plan/证据只放指针；③ 号规则 + 跨批缺陷总览落 `designs/README` 与 `docs/README` §6，跨批总览与 13 个未登记空洞的清单只在 `docs/README.md` §6.1）
> Updated: 2026-09-24（**v0.37：F215 解析链补齐** —— `pickDisplayName` 的注释承诺「主语言前缀回退」从未实现 ⇒ 管理页写入的 `zh-cn` 行匹配不到 ⇒ 中文请求落 `en`、中文界面显示英文标签名（实测 `agentic`⇒`Agentic`）。修法 = 补 `normLocale` + 前缀一级（精确优先于前缀）+ 前端同链 + 用例 5+1 + dogfood **G8.7**（含反证）；**规范层 docs/06 v1.6** 同步；零回归 **m4a 恢复 58/2**、本批 dogfood **92/0**）
> Updated: 2026-09-24（**v0.36：F214 删除前置补全** —— 服务端 `deleteLabel` 两条拒绝路径（① 有子标签 ② 任一状态挂载，**有序**）与 UI 前置逐条同面：弹窗四支文案 + 禁用条件 `hasChildren || mountCountAny > 0`；`delTarget` 改吃 `TreeRow`（顺手去 2 处 `as unknown as`）；dogfood **G8.6** 重写为**数据自适应双例**（按数据特征挑行，不写死行号 —— 上一轮就栽在写死目标上）；i18n +1 键 ⇒ **527 / +123**）
> Status: **定稿**（**2026-09-23 用户批准**）· **实现已落地（T1–T10）· 看板维重做（T6⁺）**—— 文档 8 维见 §11（§11.2 为重做轮复评）· 换靶 5 轮 + grilling 4 轮 · 门槛项已清空 · 已回填 `docs/00` §5（**v1.94**）与主 design §2.3（**v1.69**）；dogfood **92/0**（G1–G14，F215 后）；后续实现以批 plan 为准
...
| 2 | i18n 键数 | zh / en 叶子键 **各 527**（差集 **0**）· 本批 **+123 键**（404 → 527）· `board` 组 **30**（新组）· `admin` 组 **6 → 95**（+89，含 F205 补的 17 键）<br>**T6⁺ 实测（2026-09-23 重算）**：口径 = `flatten(zh)` 叶子键计数（与门禁脚本同法）· 基线 404 = 批前提交 `c44e348` 同法计数；本轮净 **−7 键**（删 `creative.*` 9 + `type.*` 2 + `trend.desc`/`label.note`/`rank.sumNote`/`hero.desc`/`hero.foot` + 孤儿键 `trend.empty.downloadsWait`，加 `trend.downloadsNote`/`label.countTitle`/`label.heatTitle`/`hero.title` + **F212 的 `labels.delete.inUseHidden`** + **F214 的 `labels.delete.hasChildren`**） |
...
### 9.8 实施期发现与处置（F203–**F216** · 实现批 T1–T10 + 收口轮）
...
| **F216** | 标签定义页写后刷新（`api/admin.ts` 的 4 个标签写函数） | **用户报缺陷（2026-09-24）**：「创建标签后列表不显示，**刷新才出现**」（编辑/删除/排序保存同病同源）。根因 = `api/client.ts` 的**语言感知 Promise 缓存**（键 `${lang} ${path}`）在标签写面**没有失效** —— 仓内其他写面均有（`api/stars.ts` → `/api/assets` · `Tokens.tsx` → `/api/tokens` · `ReviewDetail/Submissions` → `/api/reviews` · `AssetAdminCard/AssetDetail` → `/api/assets`），唯 `createLabel/updateLabel/deleteLabel/reorderLabels` **零失效调用** ⇒ 页面 `setTick` 触发的重取命中旧缓存 Promise ⇒ 界面不变；整页刷新 = 模块重建（缓存清空）⇒ 才显示。**影响面四消费者**：标签定义页自身 · 看板标签维度两图（`/api/admin/overview`）· 门户筛选 chip（`/api/labels`）· 资产卡内嵌标签名（`/api/assets`） | **定案（待执行）**：`api/admin.ts` 加内部 `invalidateLabelCaches()` = `invalidateCache('/api/labels')` + `invalidateCache('/api/admin')` + `invalidateCache('/api/assets')`，在四个写函数**成功后**调用（与 `api/stars.ts` 同范式：失效收口在 api 层）· 验证 = 缓存级直证（写后 `apiGet` 立取新值）+ 真页 E2E（创建→列表立现→改名→行内立变→删除→立即消失，**全程不刷新**）+ **反证**（注释掉失效 ⇒ 断言必 FAIL）+ dogfood 新组 G15 |
