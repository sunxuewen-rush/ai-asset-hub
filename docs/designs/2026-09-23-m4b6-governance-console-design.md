...
> Date: 2026-09-23
> Updated: 2026-09-24（**v0.39：F216 已修并验证 + 缺陷登记形态统一** —— ① `api/admin.ts` 加 `invalidateLabelCaches()`（`/api/labels`+`/api/admin`+`/api/assets`）并于四个写函数成功后调用；真页**正反双证**（标签页列表 `行 6 → 7` · 门户 chip 立现；反证：空实现 ⇒ 双双回落）· 守护 **dogfood G15**（5 条 · 自清）⇒ 断言 **92 → 97** ② 批 design 固定小节 **§9.8 实施期发现与处置** = 明细主家（plan/证据只放指针）③ 号规则落 `docs/designs/README.md` · 跨批总览落 `docs/README.md` §6.1）
