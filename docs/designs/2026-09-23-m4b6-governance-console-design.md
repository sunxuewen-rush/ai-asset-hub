...
> Updated: 2026-09-24（**v0.35：F213 门禁健壮性** —— 自测试发现 G3.5 假红：真库出现 **0 计数值的一级标签**（手动建的 `software`/`hardware`）时，图例列全行而环图不渲染 0 长度扇区 ⇒ 「扇区 == 图例」的长度相等比较必然假红。改**数据感知**（行序基准 = `overview.labels[]` · 扇区期望 = `count > 0` 行 · 补行数断言）+ 反证）
...
| **F215** | 标签名解析链（`labels/service.ts::pickDisplayName` + 管理页 `displayNameOf`） | **自测试追根发现（2026-09-24 · 用户可见）**：注释承诺回退链「locale 精确 → **主语言前缀（`zh-CN` → `zh`）** → `en` → slug」，实现第 ② 步只写 `x.locale === primary`（**只认精确 `zh`**）⇒ 管理页表单写入 `locale: 'zh-CN'`（落库归一为 **`zh-cn`**）后三步全不匹配 ⇒ **中文请求一律落 `en`**：门户/市场标签 chip 与资产卡标签名在中文界面**显示英文**（实测 `agentic` ⇒ `Agentic`、`software` ⇒ `software`）；管理页「显示名」列同因回退。触发面 = **任何经管理页建/改的标签** | ① `pickDisplayName` 补 `normLocale`（trim + `_`→`-` + 小写）与**主语言前缀**一级（精确优先于前缀）② 前端 `displayNameOf` 同源同链 ③ 用例 5 单元 + 1 端到端 ④ dogfood **G8.7** 数据自适应（真库四行 `zh*` 翻译逐条命中）+ 反证 ⑤ **规范层 `docs/06` §2.3 解析链精确化（v1.6）** |
