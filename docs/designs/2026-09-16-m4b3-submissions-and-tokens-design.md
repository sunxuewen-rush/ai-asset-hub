...
> Updated: （**v1.21：两条未闭环断言收口 —— T9④ 侧栏高亮语义 + T8③ `error.load` 孤儿键**）—— ① **T8③（拍板 ② 删键）**：`submissions`/`tokens` 两组各删 1 键（zh/en 对称 = 4 行）⇒ 键数**实测** **25 / 47 / 28 / 8**、本批净增 **82 → 80**；**Q6 改判记账见 §2.1**（原「保留」判据不成立 —— `error.load` 与 `errors.network` 是**同一次载入失败的两个说法**（互斥非互补）⇒ 接了不**；**v1.20：人工验收通过回写**—— ① 用户 **2026-09-17 实机逐条复核通过**（入口 `http://localhost:5173/login` · 账号 `m4b3_dogfood`；覆盖 = 提交页 10 项 + 令牌页 10 项 + 跨面 3 项 + 观感）② 未证项「**）
> **头部口径（2026-09-18 起）**：只留最近 1-2 版 · 不复述历史与验收数字；完整历史见 **12 修订记录**。
> SSOT：实测值与断言数 → `docs/smoke/` 证据文件 · 自检分 → 对应 design 的自检节。
> Status: **定稿**（8 维自检 ≥9 ✅ —— 定稿轮 **9.81** · v1.9 订正复评 **9.57** · v1.10 实现期订正复评 **9.79** · v1.11 复评 **9.86** · **v1.14 命名订正复评 9.89** · **v1.15 T6 落地复评 9.90** · **v1.16 造数+实证复评 9.94** · **v1.17 T7 落地复评 9.91** · **v1.18 T8 落地复评 9.92** · **v1.19 T10 落地复评 9.93** · **v1.20 人工验收回写复评 9.94** · **v1.21 未闭环项收口复评 9.95**；用户 **2026-09-16 批准**）—— 批 plan 见 `docs/plans/M4b-3-personal-submissions-and-tokens.md`；本批出口五件见 §9 与 `docs/00` §5
...
| D12 | 语义色新增（v1.5） | §4.4 色彩体系**加 1 个语义色 token `--gradient-rejected`**（v1.14 订正：命名随 C 层惯例 = `--gradient-*`，**落 C 品牌渐变层**（值只住 `aih-theme.css`）；**否决**入 A 语义补丁层 —— 该层不变式 = 实色值 + 成对 `@theme` 注册，渐变两条都不满足）= 实色**蓝→紫渐变**（#6A6DFF → #B85EFF，对标 skillhub 品牌渐变）⇒ 「已驳回」徽章 = **实底紫 + 白字**（与「已通过」的实底绿 + 白字**同构**）；「已撤回」保持 `secondary` 灰（用户 2026-09-16 定）· **全站去红**属体系级口径 ⇒ 登记 **M4b-7**（本批只覆盖两页） |
...
| 1 | Q6 | ~~**保留 `error.load`** 新键（「加载失败」≠「网络异常」）+ 复用既有 `common.retry`~~ ⇒ **2026-09-17 改判：删除**（用户拍板 ②）—— 判据订正：该键与 `errors.network` 是**同一次载入失败的两个说法**（互斥非互补）⇒ 接了只换文案不增信息；载入失败统一走 `ErrorState` 的 `tErr(code)`（`common.retry` 已复用 ✓）|
...
| 7 | 「已驳回」用**蓝渐变的紫色** | 对标 **skillhub** 品牌渐变（`--accent` = Violet `#B85EFF` · `--brand-gradient` = `#6A6DFF → #B85EFF`）；首版淡底深字**被判形态不符** ⇒ 改为**实色渐变底 + 白字**，与「已通过」同构 ✓ |
...
| 2 | `apps/web/src/i18n/zh.ts` | 新建 `submissions`（**25 键**，2026-09-17 订正：−1 `error.load`）与 `tokens`（**47 键**，同订正）两组 + `errors` 组 **+7 码**（`review.*`×6 + `token.not_found`）+ 既有 `common` 组 **+1**（`cancel`）—— 净增 **80 键**（2026-09-17 订正：`error.load` ×2 删除；**§6 为键数真值**，逐行实测；v1.15：`submissions` +`action.view`；**v1.17：`tokens` +5 scope 说明 +2 成功提示**；**v1.18：`errors` +`token.not_found`、基线订正 22 → 21**） |
...
### 6.1 新建组 1：`submissions`（**25 键** —— 2026-09-17：−1（`error.load` 删除，v1.21）；v1.7：+4（`col.type` / `type.skill` / `type.mcp` / `type.agent`）；**v1.15：+1 `action.view`**）
...
### 6.2 新建组 2：`tokens`（**47 键** —— 2026-09-17：−1（`error.load` 删除，v1.21）；v1.1：+5；v1.4：−1（`title` 复用 `dashboard.tokens`）；v1.5：按令牌页最终形态重算；**v1.17：+7**（5 个 scope 说明 + `edit.success` + `delete.success`））
...
> 2. **基线订正**：原文写「errors **22** → 28」——**逐行实测为 21 → 28**（`asset.*`×5 · `auth.*`×11 · `oidc.not_configured` · `request.invalid` · `network` · `unknown`）。净增口径不受影响（按**新增键数**计：25 + 47 + 7 + 1 = **80**；2026-09-17 订正：`error.load` ×2 删除）。
...
**双向纪律**：zh/en **键集合逐一对应**（实测口径：双向差集 = 0）；本批净增 **80 键**（2026-09-17 订正：`error.load` ×2 删除）
...
> ✅ **T10 已执行（2026-09-16）：39 PASS / 0 FAIL / `NO JS ERRORS`**（G1-G15 全绿 + 两条前置断言）。
> 运行口径两分支：`SMOKE_M4B2_PASSWORD=… SMOKE_USERNAME=<user>`（标准登录）或**复用现有登录态**（无口令时）；
> 本轮因 M4b-2 种子口令不在执行者手上，用**专用账号 `m4b3_dogfood`**（产品注册端点创建 · 零直写库）+ 复用登录态分支。
> **前置（可重放性）**：跑前重启 dev 三件（vite 长跑代理会卡 POST/PATCH/DELETE · 浏览器 CDP 会退化）；脚本强制视口 1440×1000。
> 证据：`docs/smoke/2026-09-16-m4b3-personal-a.md` §3（逐组实测表）。
...
**v1.19 复评（2026-09-16，T10 落地：dogfood 全绿 + 出口五件后）= 9.93** ✅ —— 完整性 **9.9**（G1-G15 逐组实测入档 · 出口五件状态表 · 未证项清单）· 准确性 **10**（39/39 实测；键数 26/48/28/8 逐组实测；行数 `wc -l` 权威表）· 一致性 **10**（与批 plan v0.11 对照 30/30）· 可用性 **9.9** · 追溯性 **10** · 反证 **9.9**（`error.load` 孤儿键**反证了自己的清键冲动**：它是 Q6 拍板项 ⇒ 改登记不擅删）· 边界 **9.9**（环境注记 = 可重放性前提）· 维护性 **10**。

**v1.20 复评（2026-09-17，人工验收回写后）= 9.94** ✅ —— 完整性 **9.9**（人工验收结论 + 验收期缺陷登记入档 · 未证项闭合 1 条）· 准确性 **10**（同一口令 `/sign-in/username` 200 ↔ `/sign-in/aih` 401 实测对照；库内 `account_id` 二口径实证；官方源码 `sign-up.mjs:245`）· 一致性 **10**（与批 plan v0.12 对照）· 可用性 **9.9** · 追溯性 **10**（缺陷归属提交 `fb7b4a7` 可回查）· 反证 **10**（自查 SQL 用错 join 列 `review_task.version`（重审计数）→ 已改 `asset_version_id` 并自纠入档）· 边界 **9.9**（登记「临时绕过 ≠ 修复」）· 维护性 **10**；换靶角度 = **端到端登录链路实测**（原未证项 #4 分支）+ **验收期缺陷归因**。

**v1.21 复评（2026-09-17，两条未闭环断言收口后）= 9.95** ✅ —— 完整性 **9.9**（T9④/T8③ 由「待拍板」→ 结论 + 判据入档；Q6 改判留痕）+ 准确性 **10**（键数 `25/47/28/8` **脚本实测**、zh/en 双向差集 0；删键后全仓 `error.load` 引用数 = **0** 实测）+ 一致性 **10**（与批 plan v0.13 对照；§6 表/§6.3/§3.2/真码四处同源）+ 可用性 **9.9** + 追溯性 **10**（改判理由与替代出口写明）+ 反证 **10**（**反证了自己的 Q6 原判**：原「≠ 网络异常」的判据经实测站不住 —— 两者是同一次失败的说法）+ 边界 **9.9** + 维护性 **10**；换靶角度 = **未闭环项清零**（69/69）。
> 换靶角度（本轮新增）：**「可重放性」审计**（脚本能否在他人机器上跑通：视口/口令/超时/进度四项护栏）· **「已定案 vs 实现」冲突检测**（键表定案项与实现口径重叠时，登记而非静默处置）。
...
| 版本 | 日期 | 作者 | 变更 |
|------|------|------|------|
| **v1.21** | 2026-09-17 | sunxuewen-rush | **两条未闭环断言收口**（用户拍板：T8③ ② 删键 · T9④ ① 工作台精确匹配）—— ① `error.load` ×2 删除（zh/en 对称）= Q6 **改判**（判据订正入 §2.1）⇒ 键数实测 **25/47/28/8**、净增 **82 → 80**；落地 §3.2 件表 · §6.1/§6.2 标题+表 · §6.3 口径 ② `SideNav` 精确匹配集合 = `/` + `/dashboard` ⇒ 分区父项不因子页常亮 ③ **T1-T10 断言 69/69 全闭环** ④ §11 复评 **9.95** |
| **v1.20** | 2026-09-17 | sunxuewen-rush | **人工验收通过回写**（与批 plan v0.12 / 证据文件同批）—— ① 用户实机逐条复核 ✅ ② 未证项「观感」关闭 ③ 验收期发现登录端点「本地短路」缺陷（归属 M4b-pre，另立 fix 件；本次一行数据对齐临时解锁）④ 8 维 **9.93 → 9.94** |
| **v1.19** | 2026-09-16 | sunxuewen-rush | **T10 落地：dogfood G1-G15 全绿 + 出口五件**（与代码 / 批 plan v0.11 / 证据文件同批）—— ① dogfood **39/39 + NO JS ERRORS**（脚本 885 行，两登录口径）② 遮罩路径**未证项关闭**（G9 真指针）③ 证据文件 `docs/smoke/2026-09-16-m4b3-personal-a.md` ④ 审计：`error.load` 孤儿键**登记待拍板**（Q6 定案项，不擅删）· 门户 dogfood 3 处断言缺陷已修 + 36/36 ✓ ⑤ 环境注记（跑前重启三件 + 脚本四项护栏）⑥ §11 复评 **9.93** |
...
| **v1.5** | 2026-09-16 | sunxuewen-rush | **令牌页原型评审结论全部落地**（用户：「全部都是 UI 的工作，直接聊天交流我无法准确对齐」⇒ 先做可点原型）'① **新增 §2.1d 原型评审记录**（9 轮迭代 + 8 处 AI 自查修复 + 3 条事实查证）② **§4.2 整段重写**：列 7 → **6**（名称 · Key · 权限范围 · '创建时间 · 最后使用 · 操作）· **只显有效令牌**（前端过滤）· Key = `aih_9fK2mQ4p*****3ba3`（`start` 前 12 + `metadata.tail` 后 4）'· 生效**无有效期列/无状态列** · 操作**图标化**（编辑/删除**同色**）· **新增编辑流**（改名 + 改权限）· 删除确认**去红**'· Last Used 超 3 个月 `warning` ③ **新增 D11（令牌页一揽子）· D12（语义色 `--rejected` = 蓝→紫渐变）**；清理 §2.1 **重复编号**'（旧 D7 删、旧 D8/D9/D10 → D14/D15/D16）④ **「已驳回」= 实色蓝→紫渐变 + 白字**（对标 skillhub `--brand-gradient`，与「已通过」同构；'「已撤回」保持灰 —— 用户定）⑤ **服务端改动面 3 行 → 约 44 行 / 4 文件**：+ `PATCH /api/tokens/:id` · `updateApiKey` 窄化扩 '`name`+`permissions` · `startingCharactersConfig.charactersLength: 12` · 创建写 `metadata.tail`（**仍零迁移**）'⑥ i18n：`tokens` 组 32 → **41 键**、净增 59 → **68**（键表逐行实测）⑦ dogfood +G10-G13 ⑧ §11 重评 **9.81** |
