...
> Date: 2026-09-09
> Updated: 2026-09-22（**v0.38：M4b-5 指针（历史批 · 正文不改）** —— 本文件组件树所列 `DiffNav` / `DiffView`（`market/detail/`）**已于 M4b-5 退役**（`git rm`）⇒ 改官方件薄封装 `DiffWorkspace`（`components/ui/` · 门户「版本」tab 与审核「变更对比」卡**双处挂载**）；`FileTree` / `FilePreviewDialog` 同批迁 `components/ui/`。详见 M4b-5 批 design §4.10 / §5.5。**2026-09-25 体检：头部版号补同步（原表行已含 v0.38 而头部滞留 v0.37）**）
...
> 更早版本（**v0.28–v0.35**）摘要见 **§12 修订记录** —— 头部只留最近 3 条（2026-09-21 头部卫生 · F-111 门禁修复）
...
| **v0.38** | 2026-09-22 | sunxuewen-rush | **M4b-5 指针（历史批 · 正文不改）**：本文件组件树所列 `DiffNav` / `DiffView`（`market/detail/`）**已于 M4b-5 退役**（`git rm`）⇒ 改官方件薄封装 `DiffWorkspace`（`components/ui/` · 门户「版本」tab 与审核「变更对比」卡**双处挂载**）；`FileTree` / `FilePreviewDialog` 同批迁 `components/ui/`。详见 M4b-5 批 design §4.10 / §5.5 |