...
卡片网格（20/页，对齐服务端默认 limit 20）→ 分页（**v0.27：仅 `total > 20` 时渲染** —— 单页「1 / 1」不显示；与 M4b-3 批 design §4.4.1 同口径）；响应式降列 <1200→3/<900→2。
...
| **`--gradient-rejected`**（v0.26 增 = **M4b-3 批** design D12 v1.14） | `linear-gradient(96deg, #6a6dff 0%, #b85eff 100%)` | **状态渐变**：review task「已驳回」徽章（实底蓝→紫 + 白字，**去红**）；消费 = 官方 `badge.tsx` 的 `rejected` variant （`bg-[image:var(--gradient-rejected)] text-white [a&]:hover:brightness-105`；唯一消费点 = `StatusPill kind="task"` 的映射） |

- **消费方式**：`bg-[image:var(--gradient-*)]`（Tailwind v4 任意值）——**不注册 `@theme` 映射**
  （避免与 `--color-*` 生成的 `bg-*` 工具类撞命名空间）；**值只住 `aih-theme.css`**，组件内不得出现
  裸渐变字面量。
- **白名单 = 上表 3 个 + `--gradient-rejected`（v0.26，M4b-3 批 design D12 v1.14 增）⇒ 现为 **4 个**；换皮范围内**不得**出现其它 `linear-gradient`（plan T13 断言口径据此修正）。
...
- **不翻转项（明确保持废弃）**：毛玻璃 `backdrop-filter` · 氛围光斑（`bg-glow` 节点 / hero
  `::before/::after`）· 类型色 tile 与头像仍**实底**（渐变只回品牌字 / 主 CTA / 页面底三处；v0.26 增第 4 处 = M4b-3 的「已驳回」状态徽章）。
...
- **状态语义映射**：资产 `ACTIVE=success` / `HIDDEN=warning` / `ARCHIVED=muted-foreground`；版本八态完整
  映射归 M4b（控制台 `StatusPill`——2026-09-14 拍板：`UPLOADED` = warning · `YANKED` = secondary）；门户侧
  只渲染 PUBLISHED / YANKED 徽章（`success` / `neutral`——`VersionCompare.tsx:188` 实况：tone 由「是否
  latest」决定、YANKED 仅换文案，灰度与 M4b 映射一致）。
...
**⑦ token 映射表（旧 → 新；迁移期照抄，换完即删旧文件）** —— ✅ **T24 已执行**：`src/styles/tokens.css` + `global.css` 已整体删除（样式单入口 = `index.css` + `aih-theme.css`）；旧层 `tokens.css` 共 **93** 个自定义属性；本表 **15 行覆盖 29 个 token 名**；**非注释行消费者 = 0**（实测 2026-09-11；例外三项均非旧层消费——滚动条 4 个按 §4.4 ⑥ 迁入 `aih-theme.css`、`--font-sans`/`--font-mono` 由官方层 `index.css` 的 Tailwind `@theme` 重新定义）
...
- **阶段 1 换皮完成（v0.23，2026-09-11）**：详情页（三 Tab 卡 / 右栏 · 总览 markdown · 文件树 + 预览 + 嵌套预览 · 版本对比 + 行级 diff）· 中心页（页头 + 计数 + 搜索 + 筛选条 + 4×5 网格）全量 Tailwind 化 ⇒ `.module.css` 存量 **16 → 0**（16 → 14 → 12 → 9 → 2 → 1 → 0，逐 Task 实测删除数 2+2+3+7+1+1） · 4 个共享原子件（`Badge`/`Spinner`/`EmptyState`/`ErrorState`）随 T20 增补归一 · T24 删旧层（双栈共存期结束）· 五门禁 + 全态冒烟（13 态）+ 观感用户确认 ✅；硬证据见 `docs/smoke/2026-09-11-m4a-visual-s1.md`（11 张 `s1-*.png`）。**视觉体系切换（阶段 0 + 阶段 1）至此完成**，共享壳与 token 体系可供 M4b 直接生长（无混搭期）
...
### 8.9 v0.29（2026-09-17）：首页 hero「落底 + 官方搜索组合件 + 胶囊」

**形态变更（门户面，用户 2026-09-17 逐条拍板）**：

| # | 变更 | 拍板 | 落点 |
|---|------|------|------|
| ① | **整个 hero 去卡**：`<Card>` → `<div>` ⇒ 标题 / 副标 / 搜索行 / 统计条全部**直接落在页面底色**上（`--gradient-page`）· 撑满 `flex-1` / 居中 / 内距 `px-10 py-12` / rise 动画**均不变** | B | `market/Hero.tsx` |
| ② | **搜索行改官方 `InputGroup` 框内形态**：`<InputGroup>` > `InputGroupInput` + `InputGroupAddon(align=inline-end)` + `InputGroupButton`；**M4b-1 §6.2 的 D′「输入框内按钮改 `InputGroup`」至此落地**（原「并列 ⇒ 不适用」豁免作废） | ① | 同上 |
| ③ | **胶囊化 + 单放大镜**：`rounded-full`（覆盖官方默认 `rounded-md`）· 高 `h-11` = **44px**（Google 首页同档）· **左侧 addon 撤除**，放大镜**只出现一次**且落**右侧**当提交钮（`size="icon-sm"` 32×32 · `variant="ghost"` · `type="submit"` · `aria-label`=「搜索」） | ① | 同上 |
| ④ | 底色 **保持透明**（不叠白底/投影，与「去卡」后的通透观感一致） | A | 同上 |

**③ 的依据（Google 两种范式，避免"两个放大镜"）**：
- Google **首页**大框：左侧放大镜（提示）+ 右侧**不同语义**图标（语音 / 镜头）
- Google **结果页**顶部框：**左侧不放**，右侧放大镜**即提交**
⇒ 规则：一个框内放大镜只出现一次；当它承担提交时归**右侧**。

**实测（1440×900）**：框高 **44** · `border-radius` = `calc(infinity*1px)`（真胶囊）· 框内 `svg` 数 **1** · addon 数 **1**（仅 `inline-end`）·
按钮 **32×32** `ghost` · `type=submit` 且在 `<form>` 内 · 输入框左内距 **12px** · 页面 `[data-slot=card]` **0** · hero 高 **842**（撑满）。
...
| **v0.23** | 2026-09-11 | sunxuewen-rush | **阶段 1 完成 + converge（plan T26）**：① **三向同步**——版本头升 v0.23 · §4.2 目录树 `styles/` 终态改 `aih-theme.css`（+ 单入口说明）· §4.4 ⑦ 加 ✅（旧层已删、29 个 token 名（旧层 93 属性）零消费者实测）· §8 补「阶段 1 换皮完成」段（含硬证据链接）· §11 状态同步 ② **跨文档处置**：`2026-09-08-m2-asset-domain-design` 加**失效标注行**（正文不改，史实不改）· `2026-09-10-m4b-admin-console-and-auth-design` 两处 `tokens.css` 复用点改指 `styles/aih-theme.css`（原引用会指向已删文件）③ **8 维重评 9.56**（标准 4：完整 9.5 / 一致 9.5 / 清晰 10 / 可实施 9.5；深度 4：设计纯粹 9.5 / 边界 9.5 / 实施精度 9.5 / 跨平台 9.5）④ 出口：阶段 1 四项条件全达成 |
