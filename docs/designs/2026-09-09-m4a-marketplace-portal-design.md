...
- **出口 = 用户三判「气质 / 密度 / 类型色」全部 OK** → 继续剩余页换皮；不 OK → 调 §4.4 token 后复看；
  回退面 = 壳 + 首页 + 基建（干净）
