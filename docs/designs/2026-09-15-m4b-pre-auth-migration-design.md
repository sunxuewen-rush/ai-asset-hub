...
| R16 | 依赖影响面 = **运行时新增 1 个包**（`better-auth`），**不引入任何额外运行时依赖** | 传输：Bun 运行时 + 官方声明依赖（spike 沙箱实测装得下、跑得通）；`openid-client` 与 `ldapjs` 保留（LDAP 通道与 OIDC 编排继续用）；删掉的自研面不产生新依赖 |
