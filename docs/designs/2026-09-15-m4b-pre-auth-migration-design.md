...
> Date: 2026-09-15
> Updated: 2026-09-15（**v2.4：批次收口（出口五件全绿 · 用户选 A）** —— 出口件 ④ dogfood/观感判「本批不适用 → 移交 M4b-2」（登录面未在本批交付，经代码核实为当前形态）；T8 收尾 + converge + 整体审计无未决项）
...
> 引用链：本文档 → 规范 01 §3.3 · 05 §3/§4.1/§5/§6 · 08 §3/§8 · 00 §5（引用不复制）；M4b 主 design（批件登记表与批间门）→ `2026-09-10-m4b-admin-console-and-auth-design` §2.3
...
| R9a | **业务面同源守卫**（T3 收尾补）：新增 `http/origin-guard.ts`，cookie 写请求经官方 `auth.$context.isTrustedOrigin()` 校验（**与官方同一白名单**，不重写比较逻辑）；出口沿用 `auth.csrf_failed`（07 §4）。**T8 审计修正**：「自身源」推断改读 `Host` 头（对齐官方 `getBaseURL`），修 `Origin: null` + 同站标记的误判 403（P21） | 官方 origin 校验只覆盖 `/api/auth/*`；业务面不然只剩 cookie `SameSite=Lax`（挡跨站、挡不住「同站跨源」）⇒ 相对迁移前是防线回归。语义与官方 `validateOrigin` 同构：安全方法跳过 · Bearer 显式通道跳过 · 无 `cookie` 头跳过 · `origin → referer` 回退 · `Origin: null` + 同站标记按自身源校验 · 缺 Origin 即 403 |
...
| P21 | 业务面同源守卫的「`Origin: null` + `Sec-Fetch-Site: same-origin`」回退若用**请求 URL 的 origin** 推断自身源，会在 `Host` 头与 URL host 不一致时（代理/端口改写/测试客户端）把**合法同源请求误判 403**；官方 `getBaseURL` 默认读 **`Host` 头**（`origin-check.mjs:109`） | 守卫改读 `Host` 头构造自身源（缺 Host 才回退 URL）；T8 审计新增 ⑦/⑦-b 用例（同站标记放行 · 跨站标记仍 403）锁定 |
...
| `apps/server/src/http/origin-guard.ts` | **78 行**（T8 后） | **业务面同源守卫**（R9a，T3 收尾补）：`/api/*` 除官方平面外的 cookie 写请求 → 官方 `auth.$context.isTrustedOrigin()`（同一 `trustedOrigins`）；出口 `auth.csrf_failed`（07 §4） |
...
| v2.4 | 2026-09-15 | sunxuewen-rush | **批次收口（出口五件全绿 · 用户选 A）**：① 出口件 **④ dogfood/观感 → 本批不适用（移交 M4b-2）**——用户实测「dev 点登录无响应」经代码核实为**当前形态**（`TopBar.tsx:33-39` 占位 `<span>` 无 `onClick`/`href`；`main.tsx` 路由无 `/login`；登录页归 M4b-2）⇒ 本批零 UI 改动 + 登录面未交付，UI 观感不可在本批验证 ② **复盘点**：原 H1/H2 系收尾清单设计失误（把下一批交付的 UI 面写进本批人工项），已回写 smoke §7/§8 与批 plan T8，并登记为 **M4b-2 出口件 + 立项对齐必核现状项** ③ 出口五件全绿 ⇒ 批 plan 状态 **✅ 完成**；上游 `docs/00` **v1.34** · 主 design **v1.15** |
| v2.3 | 2026-09-15 | sunxuewen-rush | **T8 收尾回写（门禁 + converge + 整体审计）**：① §2.3 增 **P21**（同源守卫「自身源」推断须读 `Host` 头——原读请求 URL 会在 Host 与 URL host 不一致时误判合法同源 403；官方 `getBaseURL` 同源做法）+ §4.1/R9a 两行同步 ② converge 8 维重评 **9.5**（全量量化声明实测回写：14 表 · FK 12 · scope 码 5 · 档位 4 · TTL 8h · 501 例）③ **整体审计（十一维全仓扫描）**：findings 逐条登记 + 处置（修 8 / 订正 0 / 回填 0 / 口径登记 6，**无未决项**）——含 7 项真死导出删除 · 3 项官方生成物保留 · 8 项「待消费/登记值」口径 · i18n 键集合对称性实测 · token 消费者逐码实测 ④ 硬证据记录 `docs/smoke/2026-09-15-m4b-pre.md` |
