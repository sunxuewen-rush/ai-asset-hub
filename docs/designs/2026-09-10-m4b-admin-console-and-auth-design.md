...
> Date: 2026-09-10
> Updated: 2026-09-17（**v1.45：M4b-2 登录默认落点 `/dashboard` → `/`（首页）（用户拍板「按推荐」）**—— ① 唯一改点 = `pages/Login.tsx` 的 `destination` 默认值（`next` 仍**优先**，被拦截后登录回原页不变）② 反向守卫（已登录访 `/login`）与成功跳转**共用该值** ⇒ 行为一致 ③ 无权限弹回（`RoleGuard`）**保持 `/dashboard`** ④ 依据 = 批 design **v1.26**（断言 **49 PASS / 0 FAIL**，+C10）⑤ 本版零实现改动；v1.44：M4b-2 官方结构 + SiteHeader 对齐 + 图标态闭环同步（用户拍板「对齐官方骨架」，并点示「相信官方积累」）**—— ① **顶栏不再横跨全宽**（宽 = 内容区宽 **1184**）· 结构改 `SidebarProvider > SideNav + SidebarInset`（顶栏入 Inset）· 侧栏回官方 `inset-y-0`（顶到最上）· 官方 container `border-r` **1px** · **AIH 覆盖点 2 → 1 处**（仅宽度变量）② 顶栏照官方 `blocks/dashboard-01` **SiteHeader** 对齐 6 项（`--header-height` 变量 · 内层容器 · `Separator mx-2` · 标题改 **`<h1>`** · `shrink-0`+`transition` · **图标态保持 58** 不收矮）③ **品牌由顶栏移入侧栏顶部** `SidebarHeader` ④ **图标态闭环**（官方规则 tw 4.3.3 未生成 = F4 真因 ⇒ 受控 open + 状态驱动）⑤ 收起态**单层底色** · hover 提示**统一纯中文** ⑥ 断言 **39 → 47 PASS / 0 FAIL** ⑦ 依据 = 批 design **v1.25**；**v1.43：M4b-2 侧栏宽 256 + 面板圆角对齐官方（用户拍板 ①保留 ②保留 ③回退）**—— ① 侧栏展开宽 **204 → 256**（官方 `16rem`；条目宽 214 · 内容区起点 x 256）② 面板圆角覆盖退役 ⇒ 官方 floating 三件套（`rounded-lg` **10px** + 1px border + `shadow-sm`）；**AIH 覆盖点 3 → 2 处** ③ **滚动条保留 AIH 6px 细蓝**（视觉体系项，不随官方原生 15px）④ 依据 = 批 design **v1.24**（实测 `sidebarW=256` `radius=10px` · 断言 **39 PASS / 0 FAIL** · 决策依据含与官方 registry 逐字符比对）⑤ 本版**零实现改动**；v1.42：M4b-2 门户组加组标题「门户」+ 14 条全中性底（用户拍板翻转 Q3）**—— ① §4 注**翻转登记**：原 2026-09-16 grilling「门户组不加组标题」经用户 2026-09-17 拍板**改为加**（新键 `navigation.groupPortal`）⇒ 四组结构自此同构（`SidebarGroup > Label + Content > Menu`）② §4 现状行同步（门户组不再「无组标题平铺」）③ §11 键数当前值 **221 → 222 键**（`navigation` 13 → 14 · 本批净增 **11**）④ **14 条衬底全中性**（资产类型色衬底撤除 ⇒ 常态 `bg-muted` / 激活 `bg-primary`）⑤ 依据 = 批 design **v1.23**（实测组标题 4 · 衬底种类 1 · 断言 **39/0**）⑥ 本版**零实现改动**；**v1.41：M4b-2 侧栏条目形态统一同步（用户拍板）**—— ① §4「门户组不加组标题」注**范围澄清**：组标题/文案/结构**仍不加不改**，但**条目级形态**经用户拍板统一（门户 4 条 **51px 双行 → 32px 单行** + 图标槽 **22×22** 衬底；英文副标降级 hover tooltip）② 依据 = 批 design **v1.22**（实测 14 条逐项相同 · 断言 **38/0**）③ 本版**零实现改动**；**v1.40：M4b-2 UI 重做落地收口（§14 定稿 · T11-T15 ✅）**—— ① §2.3 批件登记表 M4b-2 行补 **UI 视觉重做 ✅ 2026-09-17**（T11-T15 落地 · 断言 **37/0** · 门户零回归 **36/36 + chain-smoke PASS** · 证据 + 截图 5 张）② §11 键数当前值改**实测口径**：全仓 **221 键 / 11 组**（含 M4b-3 两组）· **本批 UI 重做净增 10 键**（`login` 8→16 · `device` 13→14 · `navigation` 12→13）· 双语双向差集 0③ 本版**零实现改动**（实现已在批内落地）；**v1.39：M4b-2 UI 重做 Q7/Q9/Q10 同步（顶栏标题区 · 撤 tab · 去 `AuthLayout` 页头）+ §11 键数当前值 + Status 写实**—— ① §2.4 U1 顶栏口径补 **页面标题区**（Q7）② §10.2 补三条：顶栏标题区（Q7）· 登录/设备**全屏独立版式**（Q9 撤 tab · Q10 去页头 · 语言切换右上角）· 引用批 design §14.4 ③ §11 增**键数当前值**：全仓 **134 键 / 9 组**（`login` **10 键**）· 净增 **41 → 43**（史实值保留、以本条为准）④ §12 顶栏线框加页面标题区 + **`/login` 线框改双栏版式** + 尾注补「侧栏图标真值见批 design §14.6」（引用不复制）⑤ **Status「当前进度」写实**（原停留在 M4b-1，已落后两批 ⇒ 现记 M4b-1/2/3 + M4b-pre 完成、下一批 M4b-4、M4b-2 UI 重做待定稿）⑥ 本版**零实现改动**；**v1.38：M4b-6 范围 + 「管理看板」归属登记（用户 2026-09-17 拍板）**—— ① §2.3 对齐要点表 **M4b-6** 行补 **`/admin` 管理看板**（**内容清单待该批对齐时讨论**，本表不预设）② §2.4 **U1**「管理」组补 **「管理看板」占位条目**（本批 M4b-2 只落占位：`to` 缺省 ⇒ `toast(comingSoon)` + 路由 `/admin` → `DEV_BATCH['M4b-6']` + i18n `navigation.adminBoard`；**页面本体归 M4b-6**）③ 依据 = M4b-2 批 design **§14.7**（版本以其版本头为准）+ UI 重做评审轮；本版**零实现改动**；**v1.37：M4b-3 两条未闭环断言收口**）—— ① §2.3 批件登记表 M4b-3 行键表净增 **82 → 80 键**（2026-09-17 订正：`error.load` ×2 删除 —— 用户拍板 ②；口径见批 design **v1.21** §2.1 Q6 改判）② 同批收口 T9④（侧栏高亮语义 = **工作台精确匹配**：`SideNav` 激活判定精确匹配集合 = `/` + `/dashboard`）⇒ **T1-T10 断言 69/69**③ 侧栏激活判定的落点说明：**主 design 本节（§4 入口分层）不含激活语义** ⇒ 该口径的 SSOT = 批 design v1.21 + 代码注释（`SideNav.tsx` `EXACT_MATCH_PATHS`，防漂移：本文件不复制）；**v1.36：M4b-3 人工验收回写**—— §2.3 批件登记表 M4b-3 行补 **人工验收 ✅（2026-09-17 用户实机逐条复核通过）**（出口件 ④「dogfood/观感」闭合；覆盖 = 提交页 10 项 + 令牌页 10 项 + 跨面 3 项 + 观感）· 与 `docs/00` 同源；验收期**发现一个真缺陷（归属 M4b-pre）**：自绘端点 `/api/auth/sign-in/aih` 的「本地账号短路」按 `account.account_id = 归一登录名` 查凭据行，而产品注册口径（官方 `sign-up/email`）写 `account_id = user.id` ⇒ 注册口径账号一律 **401**（与「口令错」同码）；**另立 `fix:` 件**修复（本次以一行数据对齐临时解锁）；**v1.35：M4b-3 批次完成回写**——§2.3 批件登记表补 **M4b-3 行**（该批落地中未回填）并在其「状态 / 批间门（五件）」列回写**出口五件全绿**：批 design **v1.19**（8 维 **9.93**）· 批 plan **v0.11**（§7 台账 T1-T10 全回填）· **T1-T10 ✅**（T9 并入 T10）· 提交链 11 项 `02cb290`…`3b5d7fe` + T10 收尾提交 · **dogfood 39/39 + `NO JS ERRORS`** · 五门禁 exit 0（`test` **517 pass / 1 skip / 0 fail**）· **零迁移**；证据 `docs/smoke/2026-09-16-m4b3-personal-a.md`；与 `docs/00` **v1.56** 同源；**v1.34：M4b-3 令牌面契约收紧回写**——§7.1「令牌吊销」授权行由「本人 ∨ `SUPER_ADMIN`」→ **仅本人**（令牌彻底私有，对齐规范层 `05 §5`「Token 签发 / 吊销 = 本人」；依据 = M4b-3 批 design **v1.11**）——本版不含实现改动；v1.33：M4b-3 批 design 定稿 + 登记回写**——§2.3 拆批表与批件登记表 M4b-3 行回填（批 design **定稿** · 8 维 **9.81** · 批 plan **v0.1**）· §7.1 契约表补 `reviewComment`/`assetType`/`name`/`start`/`tail`/`lastRequest` 与**新增 `PATCH /api/tokens/:id` 行** · §5.1 令牌行补编辑 ② 上游 `docs/00` 升 **v1.54**；**v1.32：M4b-2 出口五件全绿**——§2.3 登记表 M4b-2 行出口件 ④ 由「观感七项待用户
...
| **v1.43** | 2026-09-17 | sunxuewen-rush | **M4b-2 侧栏宽 256 + 面板圆角对齐官方（用户拍板 ①保留 ②保留 ③回退）**——① `--sidebar-width` 204 → **256**（官方 16rem；条目宽 214 · 内容区 x 256）② 撤除面板圆角覆盖 ⇒ 官方 floating（圆角 **10px** + 1px border + `shadow-sm`）· **AIH 覆盖点 3 → 2 处** ③ 滚动条**保留 AIH 6px 细蓝**（不随官方原生 15px）④ 依据 = 批 design **v1.24**（实测 `sidebarW=256` `radius=10px` · 断言 **39/0**）⑤ 本版零实现改动 |
