...
> Date: 2026-09-10
> Updated: 2026-09-17（**v1.37：M4b-3 两条未闭环断言收口**）—— ① §2.3 批件登记表 M4b-3 行键表净增 **82 → 80 键**（2026-09-17 订正：`error.load` ×2 删除 —— 用户拍板 ②；口径见批 design **v1.21** §2.1 Q6 改判）② 同批收口 T9④（侧栏高亮语义 = **工作台精确匹配**：`SideNav` 激活判定精确匹配集合 = `/` + `/dashboard`）⇒ **T1-T10 断言 69/69**③ 侧栏激活判定的落点说明：**主 design 本节（§4 入口分层）不含激活语义** ⇒ 该口径的 SSOT = 批 design v1.21 + 代码注释（`SideNav.tsx` `EXACT_MATCH_PATHS`，防漂移：本文件不复制）；**v1.36：M4b-3 人工验收回写**—— §2.3 批件登记表 M4b-3 行补 **人工验收 ✅（2026-09-17 用户实机逐条复核通过）**（出口件 ④「dogfood/观感」闭合；覆盖 = 提交页 10 项 + 令牌页 10 项 + 跨面 3 项 + 观感）· 与 `docs/00` 同源；验收期**发现一个真缺陷（归属 M4b-pre）**：自绘端点 `/api/auth/sign-in/aih` 的「本地账号短路」按 `account.account_id = 归一登录名` 查凭据行，而产品注册口径（官方 `sign-up/email`）写 `account_id = user.id` ⇒ 注册口径账号一律 **401**（与「口令错」同码）；**另立 `fix:` 件**修复（本次以一行数据对齐临时解锁）；**v1.35：M4b-3 批次完成回写**——§2.3 批件登记表补 **M4b-3 行**（该批落地中未回填）并在其「状态 / 批间门（五件）」列回写**出口五件全绿**：批 design **v1.19**（8 维 **9.93**）· 批 plan **v0.11**（§7 台账 T1-T10 全回填）· **T1-T10 ✅**（T9 并入 T10）· 提交链 11 项 `02cb290`…`3b5d7fe` + T10 收尾提交 · **dogfood 39/39 + `NO JS ERRORS`** · 五门禁 exit 0（`test` **517 pass / 1 skip / 0 fail**）· **零迁移**；证据 `docs/smoke/2026-09-16-m4b3-personal-a.md`；与 `docs/00` **v1.56** 同源；**v1.34：M4b-3 令牌面契约收紧回写**——§7.1「令牌吊销」授权行由「本人 ∨ `SUPER_ADMIN`」→ **仅本人**（令牌彻底私有，对齐规范层 `05 §5`「Token 签发 / 吊销 = 本人」；依据 = M4b-3 批 design **v1.11**）——本版不含实现改动；v1.33：M4b-3 批 design 定稿 + 登记回写**——§2.3 拆批表与批件登记表 M4b-3 行回填（批 design **定稿** · 8 维 **9.81** · 批 plan **v0.1**）· §7.1 契约表补 `reviewComment`/`assetType`/`name`/`start`/`tail`/`lastRequest` 与**新增 `PATCH /api/tokens/:id` 行** · §5.1 令牌行补编辑 ② 上游 `docs/00` 升 **v1.54**；**v1.32：M4b-2 出口五件全绿**——§2.3 登记表 M4b-2 行出口件 ④ 由「观感七项待用户
...
编排说明增第 **7** 批（**收尾打磨批**，前置 = M4b-1…6 全绿；**零服务端改动**）；定位 = 「**应用 M4a §4.4 体系的
最后一公里**」（节奏/密度/层次/微交互/全态一致性 + 控制台三判），**不重新设计**、体系仍以 M4a design §4.4 为
SSOT；**与「完整 converge」同批收口**；上游 `docs/00` §5 同步加 M4b-7 子行）；**v1.30：M4b-2 批次完成（T1-T10 ✅）**——§2.3 登记表 M4b-2 行由「🔵 执行中」→
**✅ 完成（2026-09-16）**，回填收尾版本（design **v1.13** · plan **v0.13**）与出口五件状态（①②③⑤ 全绿 ·
④ dogfood **36/36 + 24/24 + NO JS ERRORS** ✅ / **观感七项待用户实机确认** 🔶）；证据 = 
`docs/smoke/2026-09-16-m4b2-auth-shell.md`；**v1.29：M4b-2 批内执行进度回填（T1-T9 ✅）**——§2.3 登记表 M4b-2 行「批内进度」
T1-T8 → **T1-T9 ✅**（T10 ⬜）；依据 = 批 plan **v0.12** / 批 design **v1.12** 的 T9 落地登记（i18n 实测键数
回填：基线 **91 键** ⇒ **132 键 / 9 组** ⇒ **净增 41 键** · 补 3 个服务端实有码 ⇒ `errors` **21 键**、
服务端 12 码**覆盖 12/12** · 双语双向差集 **0** · 门户零回归 **36/36**）；
**v1.28：M4b-2 批内执行进度回填（T1-T8 ✅）**——§2.3 登记表 M4b-2 行「批内进度」
T1-T7 → **T1-T8 ✅**（T9-T10 ⬜）；依据 = 批 plan **v0.11** / 批 design **v1.11** 的 T8 落地登记（工作台临时
落地页 `pages/Dashboard.tsx` **78 行** · **三档入口裁剪 + 零业务请求 + notice toast** 全实测 ·
门户零回归 **36/36** · **1 处口径订正**：「零请求」补壳层例外（`/api/stats` 来自 M4a 既有 `SideNav`））；
**v1.27：M4b-2 批内执行进度回填（T1-T7 ✅）**——§2.3 登记表 M4b-2 行「批内进度」
...
| **M4b-7** | **控制台视觉打磨批（收尾）** | 控制台面的**视觉最后一公里**——节奏 / 密度 / 视觉层次 / 微交互 / **全态一致性**（正常·空·载·403）· **控制台三判**（气质 / 密度 / 类型色）；**镜像 M4a 视觉体系切换的做法**（探针切片 → 用户三判 → 全量打磨）；**体系仍以 M4a design §4.4 为 SSOT（引用不复制）** ⇒ 本批只做「**应用体系的最后一公里**」，**不重新设计**；**与「完整 converge」同批收口**（见编排说明） | **零** | **M4b-1…6 全绿** |

**编排说明**：
- 顺序即依赖链 **1 → 2 → 3 → 4 → 5 → 6 → 7**（M4b-1 是所有批的组件底座；M4b-2 是所有页面的认证底座；
...
- **`docs/00` §5**：M4b 行下加 M4b-1…M4b-7 子行（状态唯一源不变）
