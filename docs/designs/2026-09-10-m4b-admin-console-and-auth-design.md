...
> Date: 2026-09-10
> Updated: 2026-09-25（**v1.70：批件登记表回填（M4b-5 / M4b-6）+ 陈旧版号与 Task 数订正** —— ① §2.3「批件登记表（实际落地件与状态——每批落地后回填）」：**M4b-5 / M4b-6 两行由「⬜ 待落地回填」回填**为实际件名 / 版本 / 状态 / 批间门五件（M4b-8 保持 ⬜ 待对齐）② 同表 **M4b-1** 行 design v1.6 → **v1.8**（M4b-5 指针后真值）· **M4b-2** 行 `T1-T10` → **`T1-T15`** ③ §2.3 **预登记表** M4b-6 行「实际交付」版号 v0.33 / v0.9 → **v0.47 / v0.19** ④ **Status 段** M4b-2 五件 Task 数 → **`T1–T15`** ⑤ **§15 同版重复行合并**（`v1.65` × 2 → 1）⑥ 本笔为**跨批体检修正**（同轮：M4a design `v0.37/v0.38` 与 M4b-1 design `v1.7/v1.8` 修订表**行粘连**修复 + 头部版号同步 + `doc-audit` 增 **[F] / [F2]** 检查）。**零实现改动**）
> Updated: 2026-09-23（**v1.69：M4b-6 T6⁺ 收口（F210 门禁腐化修复）** —— ① §2.3 批件登记表 + 预登记表 M4b-6 行：批 design **v0.33** · 批 plan **v0.9** ② 验收读数：dogfood **88/0**（G1–G14）· 换靶校验 **46/0** ③ F210 = 提交后核验发现 dogfood **4 条空断言**（G2.2 文案随换靶消失恒真 · G2.3 用已不存在的原生 `select` 切档位 · G4.1/G4.2 按钮定位文案不匹配）⇒ 改「真点击 + 读真值」+ 两条反证。**零服务端改动**）
> Updated: 2026-09-23（**v1.68：M4b-6 治理批 T6⁺ 看板重做同步**（用户逐条拍板）—— ① §2.3 **批件登记表** M4b-6 行：批 design v0.29 → **v0.33**、批 plan v0.5 → **v0.9**、看板内容清单由「4 数字卡 + 趋势 + 排行榜三口径 + 创意四项 + 英雄榜 ×2」订正为「**4 数字卡 + 趋势卡（两小图）+ 标签维度两图 + 排行榜 + 英雄榜（单卡两内层分区块）**」② §2.3 预登记表同为 v0.32 / v0.8 ③ 服务端改动仍 **8 处**（T6⁺ 只加 `overview` 加性字段与两处口径纠偏，未新增端点）④ 验收读数：dogfood **84/0**（G1–G14）· 证据 §5.3。**含服务端加性改动**）
> 2026-09-23（**v1.67：顶栏回归官方 block 形态（用户拍板「甲」）+ F207 闭项** —— ① §4 顶栏行：构成 **5 → 4 件**（**撤除「页面标题区」**）—— 原自造 `titleOf` 路由表致 **F207**（漏 `/admin`、`/admin/assets` ⇒ 两页顶栏无名称）；官方无「路由 → 标题」机制（`dashboard-01/site-header.tsx` 的 `<h1>` 写死 · `sidebar-07` Breadcrumb 每页硬编码）⇒ 标题只在页内 ② §2.4 U1 / §10 变更清单 / §12 线框 同步 ③ 守护断言 = dogfood **G13**（逐路由「顶栏无 `h1` 且内容区有标题」，14 路径 + 1 聚合）· 正反双证（旧 TopBar 版本 **12 FAIL**）
> 2026-09-21（**v1.61：M4b-5 审核批 UI 口径校正（U1/U3 · 用户 2026-09-21 拍板）** —— ① **§10.2 族主文档口径按真码订正**：原文「mcp 族 `mcp.json` / `README.md`（可选）；agent 族 `agent.md` / `README.md`（可选）」⇒ 真码 `market/detail/OverviewTab.tsx:14-21` `mainDocPath` **只有两档**：`skill ⇒ SKILL.md`（必需）；**mcp / agent ⇒ `README*`**（root 级大小写归一 · 可选；缺失 ⇒ manifest 摘要回退）——**真码不探测 `mcp.json` / `agent.md`**；并补 **mcp `servers` 记录块**口径（`manifestFields` 显式滤掉 `servers` ⇒ 分型呈现自成一套，见 M4b-5 批 design §4.3）② **§12 审核详情线框补「撤回」钮**（原仅 2 钮 ⇒ 与 §10.2「通过/拒绝/撤回」及服务端 `withdraw` 端点矛盾）+ **措辞按文案真源订正**：**拒绝 → 驳回**（`apps/web/src/i18n/zh.ts:314` `review.reject = 驳回`）③ 依据 = M4b-5 批 design §4.9 / §11.2 **F152/F153**；**零实现改动**）
...
> Status: 定稿（**主 design（跨批不变层）**——拆批后本文件只保留跨批不变层（里程碑范围 / 模型与角色契约 / 路由清单 / 视觉基线归属 / 拆批表 §2.3 / 决策登记 §2.4 / 接口变更总览 §8）；批内决策另立**批 design**（命名与迁移规则见 `docs/designs/README.md`）。**主 design 定稿条件**：① **视觉体系收敛 ✅ 已闭合**（视觉真值 SSOT = M4a design §4.4，版本随 M4a 演进、以其版本头为准；控制台特有值已落值：表格密度 **40** / 抽屉宽 **560**）② **三族适用性实证 ✅ 已闭合（2026-09-22 · M4b-5 交付）**（审核详情分型 manifest 卡 · 三族断言 + 截图证据见 M4b-5 批 design §9.5/§9.8）③ **grilling ✅ 已闭环**（R1-R9 + 补充锁定 5 条）⇒ **三项条件已全闭合 · 2026-09-22 用户批准转定稿**。**当前进度（2026-09-17 回写）**：**M4b-1 ✅ · M4b-2 ✅（UI 视觉重做 §14 = 定稿候选，自检 **9.65**，⬠Q9/Q10 已闭环）· M4b-3 ✅ · M4b-pre ✅ · M4b-4 ✅ · M4b-5 ✅（审核工作台 · 2026-09-22 收口：八步门禁 **8/8 exit 0** · dogfood **62/0** · 零回归 `m4a` 34/0 / `m4b3` 43/0 / `m4b4` 89/0）** ⇒ **M4b-6 治理批 ✅ 完成（2026-09-23）**（设计定稿 8 维 9.50 ⇒ 实现 **T1–T15 全绿**：四页真页 + 服务端 8 项 + 迁移 0014 · dogfood **41/0**（T6⁺ 后 **84/0**：G1–G14）· 换靶校验 **43/0**（T6⁺ 后 **46/0**）· 证据 `docs/smoke/2026-09-23-m4b6-governance-console.md`）⇒ **下一批按依赖链 = M4b-8**；**M4b-2 的 UI 重做**待用户定稿口令后写实现代码（新增改造件 3 处 / 删件 1 处，见批 design §3.2）；其余各批批件版本与状态以 §2.3 登记表为准，批间门见 §2.3）
...
| **M4b-6** | 治理批 | **管理看板 `/admin`**（真页面 · **T6⁺ 形态**：4 数字卡 + 趋势卡两小图 + **标签维度两图**（同心环 / 雷达）+ 排行榜三口径 + 英雄榜单卡两内层分区块；原「创意四项」已按拍板删除）· **资产管理**（全站资产治理列表；侧栏「管理」组新增子项「资产管理」——2026-09-18 用户拍板，见 §2.4 U8）· 标签定义（两级树 CRUD + zh-CN/en 翻译 + 行内上/下移 + 上限提示）· 审计日志（八维过滤 + `action` 分组下拉 + 日期区间用官方 `Calendar` + 行详情抽屉） | **8 处**（§5.1–§5.4：3 只读端点 + `/api/assets` 扩参 + `download_event`/迁移 0014 + `/api/audit/actions` + 标签面 2 处 —— 原「1 处读面契约待定」已随 M4b-6 design 定稿细化） | M4b-1/2 |
...
| M4b-6 | | **M4b-6** | `2026-09-23-m4b6-governance-console-design.md`（**实际交付** · **v0.47** · 实现完成 + T6⁺ + F210/F221 收口） | `M4b-6-governance-console.md`（**实际交付** · **v0.19** · T1–T10 全绿 + T6⁺） | **`/admin` 管理看板（2026-09-17 用户拍板归属本批；内容清单待本批对齐时讨论——本表不预设）** · **资产管理（全站资产治理 + 侧栏「管理」组新条目；2026-09-18 用户拍板归属本批——路由路径与页面形态待本批对齐时定）** · 标签两级树 CRUD + 固定 zh-CN/en 翻译 + 行内上/下移（`PUT /order`）+ 上限 100 提示 · 审计八维过滤 + `action` 分组下拉（27 个动作）+ 日期区间（官方 `Calendar`）· 分页 |
...
| **M4b-5** | `2026-09-21-m4b5-review-workbench-design.md`（**定稿** · 8 维 **9.86**） | `M4b-5-review-workbench.md` | design **v0.14** · plan **v0.12** | ✅ **完成（2026-09-22）**——提交链 4 笔（`6c5cc98` / `a8152e6` / `64f3e87` / `ad05b4e`）· dogfood **62/0** · 零回归 `m4a` 34/0 / `m4b3` 43/0 / `m4b4` 89/0 · 证据 = `docs/smoke/2026-09-22-m4b5-review-workbench.md`（**2026-09-25 收口补档**：此前散在批 design §9.8 与批 plan §7）| ✅ **五件全绿** —— ① design 8 维 **9.86** ② **T1–T12 全绿** ③ 八步门禁 **8/8 exit 0** ④ dogfood **62/0** + 观感合规核对（审美归 M4b-7）⑤ **整体审计**（2026-09-25 补记 · 证据 §2）|
| **M4b-6** | `2026-09-23-m4b6-governance-console-design.md`（**定稿** · 8 维 **9.44**〔T6⁺ 复评〕） | `M4b-6-governance-console.md` | design **v0.47** · plan **v0.19** | ✅ **完成（2026-09-23）**——四页真页 + 服务端 8 项 + 迁移 **0014** · 提交 25 笔 · dogfood **108/0**（G1–G16）· 换靶校验 **46/0** · 证据 `docs/smoke/2026-09-23-m4b6-governance-console.md` | ✅ **五件全绿** —— ① design 8 维 **9.44** ② **T1–T10 + T6⁺ 全绿** ③ 八步门禁 **8/8 exit 0** ④ dogfood **108/0**（出口件 ④ 清单见批 design §9.4）⑤ **整体审计**（2026-09-25 补记 · 证据 §9）|
| **M4b-8** | `YYYY-MM-DD-m4b8-publish-design.md` | `M4b-8-publish.md` | — | ⬜ 待对齐 | ⬜ |
...
（2026-09-16 用户拍板）——顶栏只留品牌 + 侧栏触发钮 + 语言切换器（**2026-09-17 Q7 追加：+ 页面标题区** = 页面标题 14px/600 + 分区 12px）；侧栏底部同时**移除**产品元信息三项。**⚠️ v1.44 更新（2026-09-17）**：顶栏改官方 `SiteHeader` 形态 ⇒ ① **宽 = 内容区宽**（不再横跨全宽）② **品牌移出**顶栏（落侧栏顶部 `SidebarHeader`）③ 标题改语义 **`<h1 class="text-base font-medium">`**（原「标题 + 分区」自绘撤除）④ 高度取 `--header-height` 变量；**真值 = 批 design §14.4 顶栏行**（引用不复制）。**⚠️ v1.58 更新（2026-09-20 · M4b-4 验收期第五笔 T11-i）**：顶栏再加**全资产搜索框**（`size="sm"` · `<lg` 隐藏 · 回车跳 `/search?q=`）⇒ 构成 **4 → 5 件**；**规格引 M4a design §8.12**（引用不复制）。**⚠️ v1.59 更新（2026-09-20 · 用户观感微调）**：顶栏小搜索 **靠右、置于语言切换器左侧**（原 = 页面标题右侧）· 宽 **固定 `w-[320px]`** · 放大镜 **`strokeWidth=4`**（加粗一倍）—— 规格仍引 M4a §8.12。**⚠️ v1.67 更新（2026-09-23 · 用户拍板「甲」）**：③「页面标题区」**整项撤除** —— 顶栏不再渲染当前屏标题（官方 `dashboard-01/components/site-header.tsx` 的 `<h1>` 本就是**写死的 demo**、`sidebar-07` 顶栏 Breadcrumb 亦每页硬编码 ⇒ 官方**无「路由 → 标题」机制**）；标题**只在页内**（每页均有 `PageHeader` 或 `<h1>`，已逐路由实测）⇒ 构成 **5 → 4 件**；理由、实测与守护断言（dogfood **G13**）见 §4 顶栏行
...
顶栏（**构成以真码为准**：`SidebarTrigger` + 右侧「全资产搜索框 + `LanguageSwitcher`」共 **4 件**
——`TopBar.tsx:58-82`；**无页面标题区**：**2026-09-23 用户拍板「甲」撤除** —— 原自造 `titleOf` 路由表 ⇒ **F207**
（表漏 `/admin` 与 `/admin/assets`：用户 2026-09-23 报「点管理看板 / 资产管理，顶栏不显示名称」）；官方无「路由 →
标题」机制 ⇒ 标题**只在页内**渲染（每页均有 `PageHeader`/`<h1>`，逐路由实测）；守护断言 = dogfood **G13**；
**品牌已移出顶栏**、落侧栏顶部 `SidebarHeader`；本行原「只留品牌 + …」表述为
...
- **改造件**（复用载体 + 按 §2.4 U1 施工，**非零改动**——2026-09-16 订正：原「零改动」清单误列）：
  `TopBar`（**删**产品元信息三项与 `TypeIcon` 残留 → 最终形态 = `SidebarTrigger` + 右侧「全资产搜索框 + `LanguageSwitcher`」（**无标题区** —— 2026-09-23 v1.67 撤除，见 §4）——**构成以真码为准**（`TopBar.tsx:58-82`；品牌已移出顶栏、落侧栏顶部 `SidebarHeader`）；**M4b-8** 再追加「发布」入口，见 §4）·
...
- **顶栏「页面标题区」（2026-09-17 Q7；v1.44 更新）**：顶栏在「触发钮 → 语言」之间渲染**当前屏标题**；**v1.44 起**改语义 `<h1 class="text-base font-medium">`、撤「分区副标」、**品牌不在顶栏**（落侧栏顶部）、顶栏**宽 = 内容区宽**；**信息架构真值 = M4b-2 批 design §14.4 C**（引用不复制）
  **⚠️ v1.67（2026-09-23）**：本项**已撤除**（用户拍板「甲」）—— 顶栏不再渲染页面标题，页面名只在页内；见 §4 顶栏行与 §15 v1.67 行
...
```text
┌ « │ 搜索…                🌐 中|EN ┐  ← 顶栏（官方 SiteHeader 形态 · 宽 = 内容区宽）：触发钮 + **全资产搜索框（v1.58）** + 语言（**无标题区** · v1.67 撤除）
│ ⌂首页 ✦技能中心 ⚙MCP ◈专家                            │
│ ─ 个人 ─────────────────────────────────────────────  │
│  ▤ 工作台  ◫ 我的资产  ⇪ 我的提交  ⛁ 我的令牌          │
│ ─ 管理 ─（role>=10 才渲染）──────────────────────────  │
...
| 版本 | 日期 | 作者 | 变更 |
|------|------|------|------|
| **v1.70** | 2026-09-25 | sunxuewen-rush | **批件登记表回填（M4b-5 / M4b-6）+ 陈旧版号与 Task 数订正（跨批体检）** —— ① §2.3 批件登记表：M4b-5 / M4b-6 两行回填（件名 / 版本 / 状态 / 批间门五件；M4b-8 仍 ⬜ 待对齐）② 同表 M4b-1 行 design v1.6 → **v1.8** · M4b-2 行 `T1-T10` → **`T1-T15`** ③ **§2.3 预登记表** M4b-6 行「实际交付」版号 v0.33 / v0.9 → **v0.47 / v0.19** ④ **Status 段** M4b-2 五件 Task 数 `T1–T10` → **`T1–T15`** ⑤ **§15 同版重复行合并**（`v1.65` × 2 → 1 · 内容零删减留痕）⑥ 同轮：M4a design / M4b-1 design 修订表**行粘连**修复 + `doc-audit` 增 **[F] / [F2]** 检查。**零实现改动** |
| v1.69 | 2026-09-23 | sunxuewen-rush | **M4b-6 T6⁺ 收口（F210）** —— ① 批件版本推进至 design **v0.33** / plan **v0.9** ② dogfood **88/0**（G1–G14）· 换靶 **46/0** ③ F210：dogfood 4 条空断言（提交后核验由「4 张截图字节相同」牵出）⇒ G2 换官方 Combobox 真换档 + 首刻度龄硬证据 · G4 改 `data-active` + 柱数=端点非零条数 · 两条反证。**零服务端改动** |
| v1.68 | 2026-09-23 | sunxuewen-rush | **M4b-6 治理批 T6⁺ 看板重做同步（用户逐条拍板）** —— ① §2.3 批件登记表 + 预登记表 M4b-6 行：批 design **v0.32** · 批 plan **v0.8**；看板内容清单订正为「4 数字卡 + 趋势卡两小图 + **标签维度两图** + 排行榜 + **英雄榜单卡两内层分区块**」（原「创意四项」已按拍板删除）② 服务端改动仍 **8 处**（T6⁺ 只加 `overview` 加性字段 `downloads7d` + 两处口径纠偏 · F208/F208-A）③ 验收读数：dogfood **84/0**（G1–G14）· 证据 `docs/smoke/2026-09-23-m4b6-governance-console.md` §5.3 ④ 批 design 文档侧复评 **9.44**（§11.2）· 代码侧 18 维 **9.14**（重做轮首检 8.44）。**含服务端加性改动** |
| v1.67 | 2026-09-23 | sunxuewen-rush | **顶栏回归官方 block 形态（用户拍板「甲」）+ F207 闭项** —— ① **§4 顶栏行**：构成 **5 → 4 件**（**撤除「页面标题区」**）—— 原自造 `titleOf(pathname)` 路由表导致 **F207**（漏 `/admin` 与 `/admin/assets` ⇒ 两页顶栏无名称，用户 2026-09-23 报）；事实基础 = 官方 `registry:ui` **无** header/topbar 件（63 件），顶栏仅存在于 block 示例源码且标题**写死**（`dashboard-01` 的 `<h1>Documents</h1>` · `sidebar-07` 顶栏 Breadcrumb 每页硬编码）⇒ 官方**无「路由 → 标题」机制** ② **§2.4 U1** 拍板行、**§10 变更清单**「顶栏页面标题区」条目（标已撤除）、**§12 线框** 三处同步 ③ 决策副作用 = 消除「顶栏标题 + 页内标题」**同字重复**（页面名只在页内）④ 守护断言 = dogfood **G13**（14 路径「顶栏无 `h1` 且内容区有标题」+ 1 聚合「顶栏 `h1` 计数 = 0」），**正反双证**：旧 `TopBar`（含 `titleOf`）下 G13 **12 FAIL**、新形态 **16/0** ⑤ `TopBar.tsx` **110 → 82 行**（删 `titleOf` 31 行 + 3 项 import + 标题区 JSX）。**本版零服务端改动** |
| v1.66 | 2026-09-23 | sunxuewen-rush | **M4b-6 实现完成同步**：① §2.3 预登记表 M4b-6 行版本推进（design **v0.29** · plan **v0.5**）② Status 段「M4b-6 设计定稿 · 待立 plan」→ **✅ 完成（2026-09-23）**，下一批 = **M4b-8** ③ 读数与证据路径登记（dogfood 41/0 · 换靶 43/0）。**零实现改动** |
| v1.65 | 2026-09-23 | sunxuewen-rush | **M4b-6 治理批设计定稿同步** —— ① §2.3 M4b-6 行：范围补「管理看板 `/admin`」（真页面 · 原行漏记）· 服务端 **1 处 → 8 处**（明细见批 design §5.1–§5.4）② Status 段「下一批 = M4b-6」→ **M4b-6 设计定稿（2026-09-23 · 8 维 9.50）· 待立 plan**；其后按依赖链 = M4b-8 ③ 批 design 定稿件 `2026-09-23-m4b6-governance-console-design.md` · 批 plan `M4b-6-governance-console.md`（v0.1 · T1–T10）④ §2.3 预登记表 M4b-6 行文件名按实际交付订正。**零实现改动**。〔2026-09-25 体检：**同版重复行合并** —— 原两条 `v1.65` 记录（措辞稍异）合并为一条，内容零删减〕|
| **v1.64** | 2026-09-22 | sunxuewen-rush | **主 design 转定稿（用户批准）** —— 三项定稿条件全闭合后落定：① 视觉体系收敛（真值 SSOT = M4a design §4.4）② 三族适用性实证（2026-09-22 · M4b-5 交付）③ grilling（R1–R9 + 补充锁定 5 条）⇒ **Status 草稿 → 定稿**；**零实现改动** |
...
| **v1.32** | 2026-09-16 | sunxuewen-rush | **M4b-2 出口五件全绿（批次正式完成）**：§2.3 登记表 M4b-2 行「出口五件」⑤ 列由「④ dogfood ✅ / **观感七项待用户实机确认** 🔶」→ **全部 ✅**（④ = **CDP 自动断言14 PASS / 0 FAIL + NO JS ERRORS**）。**口径变更**（登记于批 design **v1.14**）：出口件 ④ 的执行方式由「用户实机逐项确认」改为 **CDP 自动断言**（七项全为功能/行为项；脚本入仓`docs/smoke/scripts/m4b2-acceptance-checklist.ts`，可重放），人工价值收敛为「**认可结论**」；用户 2026-09-16 授权代跑并**认可**。⚠️ **审美面不在本批范围**（视觉打磨归 **M4b-7**，已登记于 v1.31）。**证据**：`docs/smoke/2026-09-16-m4b2-auth-shell.md` §11。本版不含功能/契约改动（验证口径） |
| **v1.31** | 2026-09-16 | sunxuewen-rush | **新增 M4b-7「控制台视觉打磨批」（用户拍板 A）**——① **§2.3 拆批表**加第 **7** 行（**收尾打磨批**：节奏 / 密度 / 视觉层次 / 微交互 / **全态一致性**（正常·空·载·403）· **控制台三判**；**零服务端改动**；前置 = **M4b-1…6 全绿**）② **§2.3 编排说明**：依赖链 `1→2→3→4→5→6` → **`→7`**（注释：M4b-7 不改功能依赖，只在全部功能批落定后做视觉收口）· **完整 converge 与 M4b-7 同批收口** · `docs/00` §5子行 `M4b-1…6` → **`M4b-1…7`** ③ **★ 新增「视觉职责边界」四条（本版核心 —— 回答「UI 美化在哪对齐」）**：**体系层** = M4a §4.4（已完成，引用不复制）· **页面视觉定稿层** = M4b 立项**全页可点原型**（11 视图 · R9）·**逐批落地层** = 各批出口 `dogfood/观感` 只做**合规核对**（**不做审美定稿**）· **视觉收口层** = **M4b-7**（唯一美化窗口）⇒ 二者职责分离，避免每批重复对齐；某批引入新组件/新交互 ⇒ **该批内就地补录 §4.4 映射**④ **§2.3 backlog 表**加 M4b-7 行（打磨范围 / 探针切片 / 全态清单 / 偏差审计 / 逐条留痕 / 组件层收敛 /**不进本批**清单）⑤ **拍板依据**：用户 2026-09-16 就「M4b 的 UI 美化放在哪个阶段做」提问 ⇒ 逐条给出三条项目事实 + 业界三范式对照 + 推荐（M4b-7）⇒ **用户选 A**。**本版不含功能/契约改动**（拆批编排与职责边界） |
| **v1.30** | 2026-09-16 | sunxuewen-rush | **M4b-2 批次完成（T1-T10 ✅）**：§2.3 登记表 M4b-2 行状态「🔵 执行中」→ **✅ 完成（2026-09-16）**，回填**收尾版本**（批 design **v1.13** · 批 plan **v0.13**）与**出口五件**：① 批 design 8 维 ≥9（定稿 **9.44** / converge **9.50**）② **T1-T10 全绿** ③ **五门禁逐项exit 0**（CI 顺序复现：`install --frozen-lockfile` → `typecheck` → `lint` → `format:check` → `build` →`db:migrate` → **`CI=true bun run test`**；测试 = **500 pass · 1 skip · 0 fail**（501 例 / 48 文件），与M4b-pre 基线逐项一致 ⇒ 零回归）④ **dogfood**：门户 `m4a-dogfood` **36/36 + NO JS ERRORS** · chain-smoke**PASS** · **本批六组**（新建 `m4b2-auth-dogfood.ts`）**24 PASS / 0 FAIL + NO JS ERRORS** ✅ / **观感七项待用户实机确认** 🔶 ⑤ **整体审计无未决项**（十一维扫描；**F4 关闭**——实测证伪 T4 误报；行数声明订正并立权威表；旧口径指针 5 处关闭）。**证据文件** `docs/smoke/2026-09-16-m4b2-auth-shell.md`（门禁表 / dogfood六组 / 种子输出 / 审计十一维 / 行数权威表 / F4 结论 / 出口五件 / 观感清单）。本版不含设计内容改动 |
| **v1.29** | 2026-09-16 | sunxuewen-rush | **M4b-2 批内执行进度回填（T1-T9 ✅）**：§2.3 登记表 M4b-2 行「批内进度」由 T1-T8 ✅ → **T1-T9 ✅**（T10 ⬜）。依据 = 批 plan **v0.12** / 批 design **v1.12**：T9 落地（**i18n 实测键数回填**——批前基线 `0ff0693` **91 键 / 7 组** ⇒ 当前 **132 键 / 9 组** ⇒ **净增 41 键**；逐组 `login` 8 · `device` 13 · `errors` +12（18 → 21）· `navigation` +3 · `dashboard` +2 · `admin` +1 ·`common` +2 · `market`/`review` 未变；**内容改动 = 补 3 个服务端实有码** `auth.forbidden` · `auth.oidc_denied` ·`auth.oidc_state_mismatch` ⇒ 服务端 12 码**覆盖 12/12、缺失 0**；**校验全绿**：双语双向差集 **0** · 占位符不一致 **0** · 空值 **0** · 门禁四连 · 生产产物零 `M4b-` · 门户零回归 **36/36** · 服务端与协议零改动）。**本版不含设计内容改动**（进度回填与键数口径回填） |
| **v1.28** | 2026-09-16 | sunxuewen-rush | **M4b-2 批内执行进度回填（T1-T8 ✅）**：§2.3 登记表 M4b-2 行「批内进度」由 T1-T7 ✅ → **T1-T8 ✅**（T9-T10 ⬜）。依据 = 批 plan **v0.11** / 批 design **v1.11**：T8 落地（`pages/Dashboard.tsx` **78 行** = `ComingSoon` 内容槽形态 + 按档位裁剪入口 + `state.notice` 消费 ·`main.tsx` `/dashboard` 换真页 · i18n `dashboard` **+1 键**）；**六条断言实测**（**档 0 未登录 →`/login?next=%2Fdashboard`** · **三档入口裁剪**（role=1 → 2 项；10/100 → 3 项）· **零业务请求**（排除壳层`me`+`stats` 后 = 0）· 过渡件注释 · **`notice` toast + 刷新不重弹** · 门禁四连 · 生产产物零 `M4b-` ·门户零回归 **36/36** · 服务端与协议零改动）。**1 处口径订正**：「零请求」补**壳层例外**——`/api/stats`来自 `SideNav.tsx:16` 的门户组计数徽章（**M4a 既有行为**，非本页引入；消除需改门户壳 ⇒ 违反「门户零回归」⇒ 不采纳）⇒ 订正为「**页面自身**零业务请求」。**本版不含设计内容改动**（进度回填与登记） |
| **v1.27** | 2026-09-16 | sunxuewen-rush | **M4b-2 批内执行进度回填（T1-T7 ✅）**：§2.3 登记表 M4b-2 行「批内进度」由 T1-T6 ✅ → **T1-T7 ✅**（T8-T10 ⬜）。依据 = 批 plan **v0.10** / 批 design **v1.10**：T7 落地（`pages/Device.tsx` **303 行** · 新建跨页件 `components/console/AuthLayout.tsx` · `main.tsx` `/device` 换真页 ·`api/auth.ts` device 三封装 + OAuth 错误映射 · `ApiError.body` · `auth/next.ts` 的 `devicePath` ·i18n `device` **13 键**）；**十项断言实测**（四态 · **未登录保码回跳闭环** · 预填+大写归一 · 刷新态 = 终态 ·**错误体适配不落 `http_400`** · **他人已认领双路** · 门禁四连 · 生产产物零 `M4b-` · 门户零回归 **36/36** ·服务端与协议零改动）。**4 处契约订正**（★ = 设计缺口）：① ★**未登录判定机制重写**——官方 `GET /device`**未登录也 200** ⇒ 401 分流不触发且会误报「他人已认领」⇒ 改读会话三态设门 ② `expiresLabel` **无数据源**（详情接口不返 `expires_in`）⇒ 删键 ⇒ `device` **13 键** ③ 码形态 **8 位无横线** + `scope` 实测 `null`④ 「他人已认领」**改判 ② 的变体**（GET 响应缺 `client_id` / approve 403）。**本版不含设计内容改动**（进度回填与登记） |
| **v1.26** | 2026-09-16 | sunxuewen-rush | **M4b-2 批内执行进度回填（T1-T6 ✅）**：§2.3 登记表M4b-2 行「批内进度」由 T1-T5 ✅ → **T1-T6 ✅**（T7-T10 ⬜）。依据 = 批 plan **v0.9** / 批 design**v1.9**：T6 落地——新建 `pages/Login.tsx`（**205 行**）· `main.tsx` `/login` 占位改真页 · i18n`login` **+8 键** / `errors` **+9 码**（键数 39 → **56**）；**六项断言实测**（独立版式 · 两 tab（**CDP 真指针**）· **首帧骨架**（`/me` 延迟 1.6s：150-1500ms `sk=4 / form=false`，1650ms 落表单）·**错口令 inline + URL 不变** · 成功链 → `/dashboard` + `/me` 200 · **反向守卫保码回跳**`/device?user_code=ABCD-1234` + 非法 next 回落）· 门禁四连 · 生产产物零 `M4b-` · 门户零回归**36/36**。**连带修正**：**种子脚本改 upsert（A2，用户授权）**——原删+建形态在 `audit_log.actor_id`有行时恒 **23503**（引用 `"user"` 的外键 = **12 约束 / 9 张表**）；**F5 登记**（OIDC 成功`/?oidc=success` 门户面零消费 ⇒ 交 M4b 收尾 / M4c）。**本版不含设计内容改动**（进度回填与登记） |
