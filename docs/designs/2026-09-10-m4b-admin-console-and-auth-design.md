...
> Updated: 2026-09-23（**v1.67：顶栏回归官方 block 形态（用户拍板「甲」）+ F207 闭项** —— ① §4 顶栏行：构成 **5 → 4 件**（**撤除「页面标题区」**）—— 原自造 `titleOf` 路由表致 **F207**（漏 `/admin`、`/admin/assets` ⇒ 两页顶栏无名称）；官方无「路由 → 标题」机制（`dashboard-01/site-header.tsx` 的 `<h1>` 写死 · `sidebar-07` Breadcrumb 每页硬编码）⇒ 标题只在页内 ② §2.4 U1 / §10 变更清单 / §12 线框 同步 ③ 守护断言 = dogfood **G13**（逐路由「顶栏无 `h1` 且内容区有标题」，14 路径 + 1 聚合）· 正反双证（旧 TopBar 版本 **12 FAIL**）
...
> Status: 定稿（**主 design（跨批不变层）**——拆批后本文件只保留跨批不变层（里程碑范围 / 模型与角色契约 / 路由清单 / 视觉基线归属 / 拆批表 §2.3 / 决策登记 §2.4 / 接口变更总览 §8）；批内决策另立**批 design**（命名与迁移规则见 `docs/designs/README.md`）。**主 design 定稿条件**：① **视觉体系收敛 ✅ 已闭合**（视觉真值 SSOT = M4a design §4.4，版本随 M4a 演进、以其版本头为准；控制台特有值已落值：表格密度 **40** / 抽屉宽 **560**）② **三族适用性实证 ✅ 已闭合（2026-09-22 · M4b-5 交付）**（审核详情分型 manifest 卡 · 三族断言 + 截图证据见 M4b-5 批 design §9.5/§9.8）③ **grilling ✅ 已闭环**（R1-R9 + 补充锁定 5 条）⇒ **三项条件已全闭合 · 2026-09-22 用户批准转定稿**。**当前进度（2026-09-17 回写）**：**M4b-1 ✅ · M4b-2 ✅（UI 视觉重做 §14 = 定稿候选，自检 **9.65**，⬠Q9/Q10 已闭环）· M4b-3 ✅ · M4b-pre ✅ · M4b-4 ✅ · M4b-5 ✅（审核工作台 · 2026-09-22 收口：八步门禁 **8/8 exit 0** · dogfood **62/0** · 零回归 `m4a` 34/0 / `m4b3` 43/0 / `m4b4` 89/0）** ⇒ **M4b-6 治理批 ✅ 完成（2026-09-23）**（设计定稿 8 维 9.50 ⇒ 实现 **T1–T10 全绿**：四页真页 + 服务端 8 项 + 迁移 0014 · dogfood **41/0**（T6⁺ 后 **84/0**：G1–G14）· 换靶校验 **43/0**（T6⁺ 后 **46/0**）· 证据 `docs/smoke/2026-09-23-m4b6-governance-console.md`）⇒ **下一批按依赖链 = M4b-8**；**M4b-2 的 UI 重做**待用户定稿口令后写实现代码（新增改造件 3 处 / 删件 1 处，见批 design §3.2）；其余各批批件版本与状态以 §2.3 登记表为准，批间门见 §2.3）
...
| v1.65 | 2026-09-23 | sunxuewen-rush | **M4b-6 设计定稿同步**：① §2.3 批件登记表 M4b-6 行范围补 **管理看板 `/admin`**（真页面 · 原行漏记）· 服务端改动 **1 处 → 8 处** ② Status 段「下一批 = M4b-6」订正为「M4b-6 设计定稿（2026-09-23）· 待立 plan」③ §2.3 预登记表 M4b-6 行文件名按实际交付订正。**零实现改动** |
| **v1.65** | 2026-09-23 | sunxuewen-rush | **M4b-6 治理批设计定稿同步** —— ① §2.3 M4b-6 行：范围补「管理看板 `/admin`」· 服务端 **1 处 → 8 处**（明细见批 design §5.1–§5.4）② Status 段「下一批 = M4b-6」→ **M4b-6 设计定稿（2026-09-23 · 8 维 9.50）· 待立 plan**；其后按依赖链 = M4b-8 ③ 批 design 定稿件 `2026-09-23-m4b6-governance-console-design.md` · 批 plan `M4b-6-governance-console.md`（v0.1 · T1–T10）④ §2.3 预登记表 M4b-6 文件名按实际交付订正 |
