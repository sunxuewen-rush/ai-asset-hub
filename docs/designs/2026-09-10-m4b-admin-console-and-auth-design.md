...
> Date: 2026-09-10
> Updated: 2026-09-17（**v1.46：M4b-2 收尾二次审计 + 登记表状态收口** —— §2.3 批件登记表 **M4b-2 行 → ✅ 完成（2026-09-17 · T1-T15）**；v1.43 的「滚动条保留 AIH 6px 细蓝」加**推翻指针**（滚动条已收敛为原生 + 轨道 transparent，见批 design v1.27/v1.28）；**v1.45：M4b-2 登录默认落点 `/dashboard` → `/`**（用户拍板「按推荐」；依据 = 批 design v1.26））
...
| M4b-6 | `YYYY-MM-DD-m4b6-governance-design.md` | `M4b-6-labels-and-audit.md` | **`/admin` 管理看板（2026-09-17 用户拍板归属本批；内容清单待本批对齐时讨论——本表不预设）** · 标签两级树 CRUD + 固定 zh-CN/en 翻译 + 行内上/下移（`PUT /order`）+ 上限 100 提示 · 审计八维过滤 + `action` 分组下拉（27 个动作）+ 日期区间（官方 `Calendar`）· 分页 |
...
「管理」组 `role >= 10`（**管理看板**（占位条目 → 页面归 **M4b-6**；2026-09-17 拍板）+ **审核队列 + 审计浏览**）· 「超级管理」组 `role >= 100`（**标签管理 + 系统设置 + 用户管理**）——
