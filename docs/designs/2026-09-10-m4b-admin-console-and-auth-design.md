...
> Updated: 2026-09-23（**v1.65：M4b-6 治理批设计定稿同步** —— ① §2.3 批件登记表 M4b-6 行：范围补 **管理看板 `/admin`**（真页面 · 原行漏记）；服务端改动 **1 处 → 8 处**（明细见批 design §5.1–§5.4）② Status 段「下一批 = M4b-6」订正为 **M4b-6 设计定稿（2026-09-23）· 待立 plan**，其后按依赖链 = **M4b-8** ③ 批 design = `2026-09-23-m4b6-governance-console-design.md`（**定稿** · 8 维 **9.50**）· 批 plan = `docs/plans/M4b-6-governance-console.md`（**v0.1** · T1–T10）④ §2.3 **预登记表 M4b-6 行文件名按实际交付订正**（原 `…governance-design.md` / `M4b-6-labels-and-audit.md` ⇒ 实际四页口径）；**零实现改动**）
...
| **M4b-6** | 治理批 | **管理看板 `/admin`**（真页面：4 数字卡 + 趋势 + 排行榜三口径 + 创意四项 + 英雄榜 ×2）· **资产管理**（全站资产治理列表；侧栏「管理」组新增子项「资产管理」——2026-09-18 用户拍板，见 §2.4 U8）· 标签定义（两级树 CRUD + zh-CN/en 翻译 + 行内上/下移 + 上限提示）· 审计日志（八维过滤 + `action` 分组下拉 + 日期区间用官方 `Calendar` + 行详情抽屉） | **8 处**（§5.1–§5.4：3 只读端点 + `/api/assets` 扩参 + `download_event`/迁移 0014 + `/api/audit/actions` + 标签面 2 处 —— 原「1 处读面契约待定」已随 M4b-6 design 定稿细化） | M4b-1/2 |
