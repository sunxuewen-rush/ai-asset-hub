...
> Date: 2026-09-10
> Updated: 2026-09-23（**v1.68：M4b-6 治理批 T6⁺ 看板重做同步**（用户逐条拍板）—— ① §2.3 **批件登记表** M4b-6 行：批 design v0.29 → **v0.32**、批 plan v0.5 → **v0.8**、看板内容清单由「4 数字卡 + 趋势 + 排行榜三口径 + 创意四项 + 英雄榜 ×2」订正为「**4 数字卡 + 趋势卡（两小图）+ 标签维度两图 + 排行榜 + 英雄榜（单卡两内层分区块）**」② §2.3 预登记表同为 v0.32 / v0.8 ③ 服务端改动仍 **8 处**（T6⁺ 只加 `overview` 加性字段与两处口径纠偏，未新增端点）④ 验收读数：dogfood **84/0**（G1–G14）· 证据 §5.3。**含服务端加性改动**）
...
> Updated: 2026-09-23（**v1.66：M4b-6 治理批实现完成同步** —— ① §2.3 **预登记表** M4b-6 行：批 design v0.28 → **v0.29（实现完成）**、批 plan v0.1 → **v0.5（T1–T10 全绿）** ② **Status 段**当前进度：M4b-6 **设计定稿 · 待立 plan** → **✅ 完成（2026-09-23）**，下一批按依赖链 = **M4b-8** ③ 验收读数（dogfood **41/0** · 换靶校验 **43/0** · `lint` 预存在红）与证据文件路径登记。**零实现改动**）
