...
> Date: 2026-09-18
> Updated: 2026-09-20（**v1.39：F105–F107 登记 + 主 design §11 键数链补两档** —— ① **F105** 注释腐化（脚本注释仍指名已退役的 `SidebarSearch`）② **F106** §4.9 五处行数陈旧（同病根）③ **F107** 主 design §11 键数链停更两档（329 → 真值 **347**）⇒ 主 design **v1.60** 补 T11-f 335 / T11-i 347 ④ **口径补强**：收口自查 grep 清单**必含计数类活口径**（键数 / 行数 / 断言数））
...
> 更早版本（**v1.31–v1.36**）摘要见 **§12 修订记录** —— 头部只留最近 3 条（2026-09-21 头部卫生 · F-111 门禁修复）
...
| **F105** | `docs/smoke/scripts/m4a-dogfood.ts` 的 `PAGE` 作用域注释仍称壳层件为 **`SidebarSearch`**（对象已随 B″ 回退**退役**）| **注释腐化**：B″ 回退只改码与文档，**未回扫脚本注释里对已删件的指名**（同族前例：M4-pre **F17** 注释腐化 26 处）| ✅ **改述**为「顶栏全资产搜索件（常驻）」+ 保留历史注记 ⇒ **纪律**：删件 / 退役件后，回扫 `docs/smoke/scripts/**` 与源码注释中的指名 |
| **F106** | 批 design **§4.9「落地记录」5 处行数与代码脱节**：`AssetSearch` 79→**81** · `CommandPalette` 116→**128** · `TopBar` 108→**109** · `SideNav` 291→**293** · `AppShell` 98→**99** | **写进文档的行数不会自己跟着代码走**：§4.9 于 **v1.34** 一次性落数，此后件被再次触碰（A 收尾 / B′ / B″ / 观感收尾）却无人回填 | ✅ 全部 `m4b4-measure.ts` **实测订正** ⇒ **纪律**：件被再次触碰时，**回扫所有引用其行数的活口径**（与 **F107 同病根**）|
| **F107** | 主 design **§11「键数当前值」停更两档**（链尾 = 2026-09-18 T11-e **329 键**；真值 **347**）：缺 **T11-f 335**（+7 新键 −1 退役）与 **T11-i 347**（A 7 枚 + B″ 5 枚）| 同 **F72**（323 → 329 同因）：**计数类活口径在连续多轮迭代中必落后** —— 每轮只写「当轮新值」，未把「上一轮之后是否又变」纳入收口自查 | ✅ 主 design **v1.60** 补两行 + 标「**本行即当前口径**」⇒ **纪律**：收口自查的 grep 清单**必须含计数类活口径**（i18n 键数 / 件行数 / 断言数）|
| **F101/F102（同日撤回）** | 随 **B″ 回退**撤回（对象已退役）| 两行的对象 = v0.35「常驻输入框 + 紧邻下拉」一体形态（`ui/SidebarSearch.tsx` / 官方 `Combobox`）—— 用户复审后拍板回到官方对话框 ⇒ 该形态与其依赖整体退役。**保留记录以便追溯，不再作为现行纪律**；**F103/F104 保留**（Tailwind 简写不生成 / 视口须先设 —— 工具链与探针事实，与实现无关）。**另**：F102 当时的 `PAGE` 作用域修复**保留在 m4a-dogfood**（防御性 · 57/0 复跑仍通过）| ✅ **视为已闭合**（对象退役 ⇒ 无残留；`PAGE` 作用域修复保留在 `m4a-dogfood`，见 **F105** 行）|
...
| **T11-i B″ 观感收尾续**（键数链补两档 + F105–F107 登记 · 2026-09-20 · **纯文档**） | 文档 8 维 | **9.54** | 主 design §11 补 T11-f **335** / T11-i **347** 两档（链连续 **222→323→329→335→347**）· 本表补 **F105/F106/F107** 三行（原只出现在正文、**未登记**）· 顺手归一 **2 行表结构**（F100 拆格 / F101-F102 补「已闭合」格）· 口径补强 = 收口自查 grep 清单**必含计数类活口径**（键数 / 行数 / 断言数）· 扣分处 = **完整性 9.5**（本轮性质 = 补**前两轮自留缺口**，非新增价值）· **一致性 9.4**（§4.7.6 表**整体序仍混**：F90…F104 · F105-107 · 撤回行 · F100 · F93 · F92… · 键数链第三行仍带历史 `> ` 前缀）· **边界覆盖 9.4**（**F100-b** 待拍板未闭 · F96 行正文含竖线**仅登记未修**）· 代码 18 维 **N/A**（零代码改动）|
...
| **v1.39** | 2026-09-20 | sunxuewen-rush | **F105–F107 登记（本轮自查发现）+ 主 design §11 键数链补两档** —— ① **F105**（注释腐化：`m4a-dogfood.ts` 的 `PAGE` 注释仍指名已退役件 `SidebarSearch`）② **F106**（§4.9 落地记录 **5 处行数陈旧**：`AssetSearch` 79→81 · `CommandPalette` 116→128 · `TopBar` 108→109 · `SideNav` 291→293 · `AppShell` 98→99）③ **F107**（主 design §11 键数链停更两档：链尾 329 → 真值 **347**，缺 T11-f **335** 与 T11-i **347**；**同族前例 F72**）④ 主 design **v1.60** 补两行 + 修订记录 ⑤ **口径补强**：**收口自查的 grep 清单必须含计数类活口径**（i18n 键数 / 件行数 / 断言数）—— F106 与 F107 是**同一病根的两种表现** ⑥ **F-表 2 行结构补齐**（`§4.7.6` 表头 = 4 列）：**F100 行拆格**（现象 / 细节分离）· **F101/F102 行补「✅ 视为已闭合」格** —— 与 F96/F100 同族（修订/登记表**列数完整性**） ⑦ **零代码改动** ⑧ **自检（本轮）**：**文档 8 维 9.54**（标准 4 = 完整性 9.5 / 一致性 9.4 / 清晰度 9.7 / 可实施性 9.6；深度 4 = 设计纯粹性 9.5 / 边界覆盖 9.4 / 实施精度 9.7 / 跨平台 **N/A 剔除**）· **代码 18 维 = N/A**（本轮零代码改动）|
