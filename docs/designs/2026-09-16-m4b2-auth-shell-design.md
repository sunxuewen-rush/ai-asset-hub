...
| 4 | `api/client.ts` | 增 **`apiPost`**（复用 `doFetch`）｜**401 单点分流**（`doFetch` 错误分支）｜`ApiGetOptions` 与 `apiPost` 选项各增 **`skipAuthRedirect?: boolean`**（§4.2 ④）｜导出 **`invalidateCache(prefix?)`**｜`sanitizeNext` 落 **`auth/next.ts`**（件 9，不放在本文件） | 主 design U3 · N2 |
