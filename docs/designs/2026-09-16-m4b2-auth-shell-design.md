...
> · 头部与修订记录**不复述验收数字**（断言数 / 门禁结论 / 自检分 / 键数 / 实测值）
> · 这些数字的 **SSOT**：实测值与断言数 → `docs/smoke/` 证据文件 · 自检分 → **§12.2** · i18n 键数 → **§10** · Task 状态 → 批 plan
> · 头部与修订记录只讲「**改了什么、为什么**」；变更**性质**里的值（如「宽 204 → 256」）属于变更本身，保留
> Status: **定稿**（8 维自检 **≥9**，实测值与历轮口径见 §12.2）· 上游主 design `docs/designs/2026-09-10-m4b-admin-console-and-auth-design.md` §2.3 的子批之一（其版本随自身演进，**以其版本头为准**）· **UI 视觉重做：§14 定稿并落地（T11-T15 ✅）**· 收尾二次审计无未决项（见批 plan §3）
