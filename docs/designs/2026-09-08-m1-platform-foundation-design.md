...
> ⚠ **后继变更指针（2026-09-15 登记 · 不改本节结论）**：本节的会话存储与 CSRF 校验链已列入
> **M4b-pre（认证整车迁移）**——见 `docs/00` §5 追踪表 M4b-pre 行；本节结论（含「Session 进程内内存」
> 与「Origin → Referer → 皆缺拒绝」链）为 M1 阶段决策档案**原貌保留**，迁移后形态以该批 design 为准。
...
> ⚠ **后继变更指针（2026-09-15 登记 · 不改本节结论）**：§6.3 API Token / §6.5 OIDC / §6.6 Device Flow
> 的实现形态属 **M4b-pre（认证整车迁移）** 范围（`docs/00` §5 M4b-pre 行）；本节结论原貌保留。
...
| v1.3 | 2026-09-15 | sunxuewen-rush | **认证面后继变更指针**（用户 2026-09-15 拍板「加指针」）：§5 与 §6 顶部各加一行指针，指向 **M4b-pre（认证整车迁移）**（`docs/00` §5 追踪表；2026-09-15 拍板前置、spike 待跑）——**只加指针，不改本节任何结论**（本档为 M1 阶段决策档案，保持原貌；迁移后形态以后续批 design 为准）。同步 `docs/00` 升 **v1.29** · 主 design 升 **v1.11** |
