...
> 并补记「翻页回顶」由契约-代码不一致 → 补实现 ③ §3.12 面包屑预期「⚪ 零变化」→ 实测 **4 处官方默认差异**
> （字阶 / 分隔符 / 链接色 / 当前项语义），用户拍板「全按官方」④ 三处均**只订正预期列为实测值**，实现侧零改动
> （订正依据 = 批 plan 各 Task 执行期说明的实测证据）；**v1.4：T8 收尾 —— 门禁/冒烟/§9 复验结果登记（批完成）**——① 五门禁逐项 exit 0
