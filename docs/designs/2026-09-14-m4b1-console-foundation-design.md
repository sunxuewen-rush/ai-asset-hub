...
> Date: 2026-09-14
> Updated: 2026-09-22（**v1.8：M4b-5 指针（历史批 · 正文不改）** —— 本文件 §3 件表所列 `DiffNav` / `DiffView`「**保留手搓**」条目**已被 M4b-5 翻转**：两件退役（`git rm`）· 改官方件薄封装 `DiffWorkspace`（`components/ui/`）；件表其余项不变。**2026-09-25 体检：头部版号补同步（原表行已含 v1.8 而头部滞留 v1.7）**）
...
| **v1.8** | 2026-09-22 | sunxuewen-rush | **M4b-5 指针（历史批 · 正文不改）**：本文件 §3 件表所列 `DiffNav` / `DiffView`「**保留手搓**」条目**已被 M4b-5 翻转** —— 两件退役（`git rm`）· 改官方件薄封装 `DiffWorkspace`（`components/ui/`）；件表其余项不变 |
