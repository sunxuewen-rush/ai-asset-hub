...
| 批 | 主题 | 范围要点 | 服务端改动 | 前置 |
|----|------|---------|-----------|------|
| **M4b-1** | **地基批：组件归位 + 控制台组件面域** | 手搓展示件 → 官方件 14 处 · 官方件补装 13 件 · `components/console/` 6 件 · 跨面 4 件 · 合规清理 4 项 · i18n 骨架 | **零** | — |
...
- 顺序即依赖链 **1 → 2 → 3 → 4 → 5 → 6**（M4b-1 是所有批的组件底座；M4b-2 是所有页面的认证底座）；
  3/4 与 5/6 之间可微调先后
- **文档形态**：umbrella（本文件）保留模型/契约/路由/视觉基线；**每批另立 design 块 + plan 文件**
  （命名 `docs/designs/YYYY-MM-DD-<批>-design.md` / `docs/plans/<批>-<主题>.md`）
- **出口口径**：每批「自检 ≥9 + 五门禁 + dogfood」；**完整 converge 放到 M4b 全部子批收尾**（避免六次重评）
