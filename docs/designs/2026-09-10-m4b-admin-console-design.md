...
- **出口口径**：见下行「批间门」（四件全绿）；**完整 converge 放到 M4b 全部子批收尾**（避免六次重评）
- **`docs/00` §5**：M4b 行下加 M4b-1…M4b-6 子行（状态唯一源不变）
- **批间门（gate）**：上一批**出口四件**全绿才启下一批 —— `批 design 8 维 ≥9 定稿` + `批 plan Task 全绿` + `五门禁` + `dogfood/观感`；未达标不进入下一批（各批 plan §1「前置」引用本条）
