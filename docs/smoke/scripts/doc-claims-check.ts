...
const NUMBERS: { label: string; expected: string; re: RegExp; group?: number; allow?: string[] }[] =
  [
    ...
    // （「i18n **+122** 键」）；而 §9.7 表行形态（`| 2 | i18n 键数 | … 本批 **+122 键** …`）因行内 `|` **永不命中**
    ...
    // ⇒ 删键后文档 `+130 / 各 534 / board 39` 全漂移而门禁仍假绿。补**绝对值锚**两条（真值 = `flatten(zh)` 实数）：
    ...
    {
      label: 'i18n board 组键数',
      expected: '30',
      re: /`board`\s*组\s*\*{0,2}(\d{1,3})/g,
    },
  ];
