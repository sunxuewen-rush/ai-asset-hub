...
const detail = await json('/assets/smoke-ns/demo-rag-skill');
const d = detail.body;
...
const vl = await json('/assets/smoke-ns/demo-rag-skill/versions');
...
const vd = await json('/assets/smoke-ns/demo-rag-skill/versions/1.1.0');
...
const fc = await fetch(`${base}/assets/smoke-ns/demo-rag-skill/versions/1.1.0/files/SKILL.md`);
const text = await fc.text();
ok('文件内容 200 + v1.1 特性', fc.status === 200 && text.includes('模糊前缀匹配'));
ok('文件内容含 md 表格', text.includes('| topK |'));

const cmp = await json('/assets/smoke-ns/demo-rag-skill/versions/compare?from=1.0.0&to=1.1.0');
...
const eq = await json('/assets/smoke-ns/demo-rag-skill/versions/compare?from=1.1.0&to=1.1.0');
ok('from=to → 200 空 files', eq.status === 200 && eq.body.files.length === 0);

const dl = await fetch(`${base}/assets/smoke-ns/demo-rag-skill/versions/1.1.0/download`, { redirect: 'manual' });
...
const mcp = await json('/assets/smoke-ns/demo-http-mcp');
ok('mcp 详情 latestVersion 1.0.0', mcp.body.latestVersion === '1.0.0' && mcp.body.type === 'mcp');
const mcpFc = await fetch(`${base}/assets/smoke-ns/demo-http-mcp/versions/1.0.0/files/README.md`);
