...
> 关联：批 design `2026-09-14-m4b1-console-foundation-design` **v1.4**（§8/§9）· 批 plan `M4b-1-console-foundation` **v0.12** ·
...
- **批 design 三处预期列待批末 converge 回改**（T5/T6 登记）：§3.7 `Spinner`「同为环形」、§3.11 分页
  「原为两端分布 / `mx-auto` 生效」、§3.12 面包屑「⚪ 零变化」（实测均不符，实测值见批 plan 各 Task 执行期说明）。
