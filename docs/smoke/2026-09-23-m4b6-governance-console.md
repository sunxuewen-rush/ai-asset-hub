...
| `m4a-dogfood.ts` | **58 PASS / 2 FAIL** + `NO JS ERRORS`（**2026-09-24 造数复位后复跑：不变** = 与造数无关） | `diff 三型徽章` / `diff +/− 行内容` —— 变更对比面，需**两份可比版本**前置 ⇒ **数据态** |
...
| `m4b4-personal-b-dogfood.ts` | **83 PASS / 6 FAIL**（复位后**仍 6** —— 根因已查明，见下「复位留痕」） | ① `G6 ownRows=3 · 全表 rows=5`（断言只认本批 `m4b4-seed-` 行）② `G8 匿名可下载`（无受控下载链）③ `G12 {"del":3,"yank":0}`（**无 PUBLISHED 行可撤回分发**）④ `G12b-1/2` = ③ 的**级联** ⑤ `G20-9 rows:11`（断言期望 6 行）⇒ 全部**数据态**；其中「全表行数」漂移**含本批种子资产**（`m4b6-seed-downloads`）⇒ 属造数叠加，非代码路径 |
...
> 与 F213/G3.5 同类）。**修法（待授权）**：跑 `m4b4-seed-assets` 复位版本态与 latest 指针 + `G20-9` 断言改
> **数据自适应**（按端点真值比对，不写死 6）。
