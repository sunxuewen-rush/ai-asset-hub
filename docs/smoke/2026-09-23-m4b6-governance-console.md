...
> **⑥ 体检修正（2026-09-24 · 跨批陈旧措辞 2 处 · 非事实变更）**：① 本文档 §5 的 **F216** 行处置栏「**定案（待执行）**」→「**已修并验证**」（`2a25fa2`）② 前序批 plan `docs/plans/M4b-4-me-assets-and-console.md` **§7 导语**「待执行。模板（…）」→「**执行期回填**（2026-09-20 起逐条追加；格式对齐 M4b-1 / M4b-3 先例）—— 原导语「待执行」于 2026-09-24 跨批体检修正」—— 两处**内容早已落地/回填**，仅措辞陈旧；不动历史批版本头与修订表（保持「头部 ≤3 行」门禁）。
> **⑦ 体检补记（2026-09-25 · 跨批）**：补**整体审计**（出口五件第 ⑤ 件 —— 本批收口以「收口轮」落地但**未按该名目留痕**）⇒ 见 **§9**；同轮修复项（修订表行粘连 2 处 · 批间门台账回填 · 陈旧版号订正）见本批设计 **v0.47 之后的主 design v1.70** 与 `docs/smoke/2026-09-22-m4b5-review-workbench.md` §2。
>
...
| **F216** | 标签写后刷新（`api/admin.ts`） | **用户报缺陷（2026-09-24）**：「创建标签后不显示，刷新才出现」。根因 = `api/client.ts` **语言感知 Promise 缓存**在标签写面**漏失效**（其他写面均有）⇒ `setTick` 重取命中旧 Promise；整页刷新 = 模块重建才见。影响面四消费者：标签页自身 / 看板标签维度 / 门户 chip / 资产卡内嵌标签名 | **已修并验证**（2026-09-24 · 体检轮修正陈旧措辞）：`api/admin.ts` 加 `invalidateLabelCaches()`（`/api/labels` + `/api/admin` + `/api/assets`）在四个写函数**成功后**调用（同 `api/stars.ts` 范式）· 验证 = 缓存级直证 + 真页 E2E（创建→立现→改名→立变→删除→立隐，全程不刷新）+ **反证** + dogfood G15 —— 明细见批 design **§9.8** |
...
## 9. 整体审计（十一维 · **2026-09-25 补记**）

> **为何补记**：本批收口（2026-09-23/24）以「收口轮」形态落地（§5.8 / §5.9 / §5.10 + F203–F221 + §8 未证项 10 项），
> 但 plan / design / 本件均**未出现「整体审计」名目**（与 M4b-5 同因）⇒ 跨批体检（2026-09-25）补记。
> **十一维逐维结果 = 同一轮实跑**，见 `docs/smoke/2026-09-22-m4b5-review-workbench.md` **§2**（**引用不复制**）；本节只记**本批面差异**。

| 维度 | 本批面结果 | 处置 |
|------|-----------|------|
| 文档门禁 / 版本指针 | 批 design **v0.47** · 批 plan **v0.19** 与各自头部一致（`doc-claims-check` [6] PASS）；`docs/00` §5 行 **v1.94**；本批行读数 dogfood **108/0** 与证据一致 | 已绿 |
| 修订表行结构 | 本批两件（design / plan）**[F] PASS** —— 无粘连 | 无需动作 |
| 批间门台账 | 本批行（主 design §2.3「批件登记表」）由「⬜ 待落地回填」→ **已回填**（主 design **v1.70** · 2026-09-25） | **修** |
| 计划头陈旧指针 | 批 plan 头：上游 design `v0.30 · 8 维 9.50` → **`v0.47 · 9.44`〔§11.2 T6⁺ 复评〕** · Status `v0.41` → **`v0.47`** · 主 design `v1.65` → **`v1.70`** · `docs/00` `v1.91` → **`v1.94`** | **订正** |
| 已删件残留 / 注释腐化 | 本批转正后 PoC 清理（T10 步骤 7：`pages/__proto/` 4 页 + 3 数据文件 + `apps/server/tmp-*.ts` 3 件）**零残留**（`git status` 无差异） | 无需动作 |
| 死导出 | 与全仓同轮结论一致（9 个候选 · 口径登记；`FilterBar` 整件归 M4b-7） | 口径登记 |
| i18n | zh = en = **536 键**（本批 **+132**）· 双向差集 **0** · 泄漏 **0** | 无未决项 |
| 既有登记项状态 | **F203–F221 全部有处置**（§5 表 + §5.1–§5.10）· `doc-audit` [E] 总览覆盖 **208** 个 F 号 | 无未决项 |
| 官方件硬规则 / 旧口径指针 | 同轮扫描：手搓遮罩 0 命中 · `worker` 零违规（唯一命中为 Web Worker 技术词） | 无需动作 |
| 越轴值 / token 消费者 / 类串重复 | **未做**（需对照 M4a design §4.4 轴值表逐项判定） | **登记 → M4b-7** |

**本批结论**：出口五件 ①②③④ 有据、**⑤ 本轮补记完成**；余「越轴值」一维按归属登记 **M4b-7**，无本批遗留未决项。
