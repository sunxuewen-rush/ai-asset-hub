...
| G8 | 标签定义页（**超管**） | 端点 200 · 上限块在位（`total`/`limit`）· 六列在位 · 首行 ↑ 禁用 · **F212 两条**：出参 `mountCountAny ≥ assetCount` 逐行不变量 · 删除确认（有任一状态挂载 ⇒ 确认钮禁用 + 文案三段口径） |
...
> ⚠️ **环境备注（2026-09-24 · 自测试）**：真库被**手动编辑**过 —— 新增两条一级标签 `software` / `hardware`
> （0 计数值 · 13:50 由超管账号创建，仓内代码/测试/seed **无此字面量**）且 `agentic` 的 **zh 显示名被改为「Agentic」**
> ⇒ 门户口径的 `m4a` dogfood 有 2 条断言（按中文精确匹配 chip「智能体」）因此**FAIL（58/2 → 56/4）**，
> **与本批代码无关**（`/api/labels` 公开面未改）；其余三脚本 `m4b3 38/3 · m4b4 83/6 · m4b5 52/10` 与 T10 基线**逐项一致**。
> 建议（待拍板）：把 `m4a` 的 chip 断言改为按 slug 匹配（对显示名漂移免疫）。
