...
| M4 | 前端：市场门户（类型化浏览/搜索/详情）+ 管理后台 | M4a ✅ 2026-09-09（市场门户：类型化浏览/搜索/详情三 tab + 行级版本对比——design/plan 见 `2026-09-09-m4a-marketplace-portal-design` + `M4a-marketplace.md`；T1-T19 全绿 538 tests；联调 dogfood 截图/记录 docs/smoke/2026-09-09-m4a-t18.md；修复录：useApi StrictMode abort / server lint 债全清）。**M4a 视觉体系切换 ✅ 完成（2026-09-11 · 阶段 0 + 阶段 1）**：全站统一 **shadcn 蓝科技**（Tailwind v4 + shadcn CLI，base=radix），**顺序为先门户后控制台**（翻转 2026-09-10「先控制台面」——理由：共享壳 AppShell/TopBar/SideNav 为门户与控制台共用，先换壳消除混搭期；门户的类型色/卡片/hero 是 shadcn 体系最难落的一块，前置解决）→ design **v0.24**（§4.4 = **全站视觉真值 SSOT**：色值/圆角轴/字阶/组件真值 + token 映射表 + AIH 补丁表）；**阶段 0 探针切片** = 基建落仓 + 共享壳 4 件 + 首页 5 件（**代码完成 ✅ + 三判通过 ✅ 2026-09-11**：五门禁全绿——全量 **475 例 474 pass / 1 skip / 0 fail**；双冒烟 30/30 + 22/22；自检记录 `docs/smoke/2026-09-11-m4a-visual-s0.md`；**用户三判「气质 / 密度 / 类型色」三项 OK**（T14 出口达成）→ **评审控件已撤除**（grep 断言 0）→ **阶段 1 完成 ✅**（T17-T26：详情页 / 中心页换皮 · design 正文语义回写 18 行 · 删旧层（`.module.css` 16→0、`tokens.css`/`global.css` 删除、样式单入口）· 五门禁 **475 例 474 pass / 1 skip / 0 fail** · 全态冒烟 **36/36 + NO JS ERRORS** · 用户观感确认 OK · converge 8 维重评 **9.56**；硬证据 `docs/smoke/2026-09-11-m4a-visual-s1.md`）→ 定稿。M4b ⬜ 管理后台（**认证登录前端**（登录页 / 会话上下文 / 401 拦截 / 角色感知）+ 治理面 UI（审核队列 / 标签 / 资产生命周期 / 令牌 / 审计）——按扁平模型重写 design v1.3，待立项；**用户管理已移出 → M4c**；**依赖 M4a 视觉体系切换先行**：控制台在已实证的 shadcn 组件体系与 token 上生长，不再有双栈混搭期） |
...
| **M4c** | **账号与权限治理**：用户管理（列表 / 改角色 / 启用·禁用 / 管理员建号）· 准入策略 `ACCESS_POLICY`（PENDING → 审批）· 重置密码 · 强制登出（Session 按用户吊销）· Device Flow 确认页（归属待定：M4c 或随 M5 CLI——立项时定档） | ⬜（**依赖 M4b 控制台组件基建**：DataTable / Drawer / ConfirmDialog / StatusBadge 等；用户 2026-09-10 拍板 A 拆出——原 M4b design R3「用户管理不入首期」随之翻转。**范围档位候选（立项时定档）**：A 最小闭环（列表 + 改角色 + 启用/禁用）· A+ 追加管理员建号 · B 追加重置密码 / 强制登出 · C 追加待审批 PENDING（依赖 `ACCESS_POLICY` 实现）） |
| M5 | CLI：安装/发布/搜索（对齐 OpenSkills/ClawHub 习惯） | ⬜（**Device Flow 确认页归属未定**——`M1-phase2` plan 原记「M4 web」，2026-09-10 决策改为「M4c 或随 M5 CLI」；**M5 立项前需与 M4c 对齐定档**，避免两侧都假设对方承接） |
...
| v1.17 | 2026-09-11 | sunxuewen-rush | **M4a 视觉体系切换 · 阶段 0 收尾同步（跨文档版本引用校正 + 完成注记）**：① §5 M4a 行——设计引用 `design v0.9` → **`v0.20`**（本工作单元内 v0.10-v0.20 连续演进：AIH 蓝基色层 / 侧栏原语 / 三判前微调 v0.18-v0.20）；补 **「阶段 0 代码完成 ✅」** 注记（五门禁全绿：全量 **475 例 474 pass / 1 skip / 0 fail**；双冒烟 30/30 + 22/22；自检记录 `docs/smoke/2026-09-11-m4a-visual-s0.md`）② §5 M4b 行——`design v1.2` → **`v1.3`**（视觉体系对齐版）③ M4b design 内 2 处 `M4a design v0.9 §4.4` 引用升 **v0.20** 并加「版本随 M4a 演进，以 M4a 版本头为准」（防跨文档版本引用再漂移——此次漂移即由固定版本号引用引起）④ M4a design 回写 **v0.18→v0.20**（hero 与侧栏对齐 + 去硬编码 / 去中间分割线 / 统计条改卡片撑满）+ plan 升 **v0.8**（三块落地记录 + T13/T15 结果 + §1 内部矛盾修复）；**自检发现**：plan §1「本阶段零服务端改动 / 不改测试」被 v0.17 打破、M4a design 引「用户数量」而 00 仍写「原生类型」类跨文档漂移同轮清 |
