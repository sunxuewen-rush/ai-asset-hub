...
> Date: 2026-09-04
> Updated: 2026-09-10（v1.15：**§5 里程碑调整（用户 2026-09-10 拍板 A）**——新增 **M4c 行**（账号与权限治理：用户管理 / 准入策略 ACCESS_POLICY / 重置密码 / 强制登出）；M4b 行范围明确（**认证登录前端** + 治理面 UI，**用户管理移出 → M4c**）+ M4c 依赖 M4b 控制台组件基建注记；v1.14：§5 M6 行注记——**CI 基础流水线提前交付**（`.github/workflows/ci.yml`：install → typecheck → lint → format:check → build → db:migrate → test，push main + PR 触发）；v1.13：**M4-pre 扁平化重构同步**——§2.2 L50/L51-52 坐标与审核主体（去命名空间/空间管理员）；§5 新增 **M4-pre 里程碑行**（✅ 2026-09-10）+ M4 行 M4b 范围更新；§6 首期非目标补「多租户/空间隔离」；v1.12：§5 M4 行注记——M4a 市场门户完成 ✅（2026-09-09 收尾：portal design v0.8 定稿落地，T1-T19 全绿 538 tests，联调 dogfood 记录 docs/smoke/2026-09-09-m4a-t18.md）；M4b 管理后台拆为待办 ⬜）；v1.11：§5 M3 行完成注记（治理管线闭环 2026-09-08）；v1.10：§5 M2 行完成注记（资产域闭环 2026-09-08）+ M3 行治理语义注记；v1.9：§7 ② 设计层 8 段 → 7 段骨架（删任务清单段）；v1.8：§7 ② 补实施完成重评（converge）约定；v1.7：M1 阶段二完成注记；v1.6 Status 同步定稿；v1.5 M1 阶段一 platform-core 完成注记；v1.4 M6 补 CHANGELOG/runbook）
