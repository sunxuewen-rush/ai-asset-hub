...
| `agent` | agent 定义包（声明型：行为描述 + 模型偏好 + 技能引用；无执行代码） | agent 运行时配置目录（各客户端自定） |
...
协议规范：`00` 本文档 · `01` 资产协议总纲 · `02+` 各族协议（skill/mcp/agent）
设计文档：`YYYY-MM-DD-<主题>-design.md`（阶段性设计，含 8 段骨架）
命名/元信息/修订记录规则沿用团队沉淀的文档习惯；语言：中文为主，术语保留英文。
...
| v1.1 | 2026-09-04 | sunxuewen-rush | 类型三件套定为 skill/mcp/agent（worker 退役）；演进路径中立化；文档内部语境清理 |
