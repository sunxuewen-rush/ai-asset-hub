...
| M4 | 前端：市场门户（类型化浏览/搜索/详情）+ 管理后台 | M4a ✅ 2026-09-09（市场门户：类型化浏览/搜索/详情三 tab + 行级版本对比——design/plan 见 `2026-09-09-m4a-marketplace-portal-design` + `M4a-marketplace.md`；T1-T19 全绿 538 tests；联调 dogfood 截图/记录 docs/smoke/2026-09-09-m4a-t18.md；修复录：useApi StrictMode abort / server lint 债全清）。**M4a 视觉体系切换进行中 ⬜（2026-09-11 用户拍板）**：全站统一 **shadcn 蓝科技**（Tailwind v4 + shadcn CLI，base=radix），**顺序为先门户后控制台**（翻转 2026-09-10「先控制台面」——理由：共享壳 AppShell/TopBar/SideNav 为门户与控制台共用，先换壳消除混搭期；门户的类型色/卡片/hero 是 shadcn 体系最难落的一块，前置解决）→ design **v0.20**（§4.4 = **全站视觉真值 SSOT**：色值/圆角轴/字阶/组件真值 + token 映射表 + AIH 补丁表）；**阶段 0 探针切片** = 基建落仓 + 共享壳 4 件 + 首页 5 件（**代码完成 ✅ + 三判通过 ✅ 2026-09-11**：五门禁全绿——全量 **475 例 474 pass / 1 skip / 0 fail**；双冒烟 30/30 + 22/22；自检记录 `docs/smoke/2026-09-11-m4a-visual-s0.md`；**用户三判「气质 / 密度 / 类型色」三项 OK**（T14 出口达成）→ **评审控件已撤除**（grep 断言 0）→ **阶段 1 进行中**（剩余页换皮 + 17 处 M4-pre 语义回写，任务细化待落）→ 定稿。M4b ⬜ 管理后台（**认证登录前端**（登录页 / 会话上下文 / 401 拦截 / 角色感知）+ 治理面 UI（审核队列 / 标签 / 资产生命周期 / 令牌 / 审计）——按扁平模型重写 design v1.3，待立项；**用户管理已移出 → M4c**；**依赖 M4a 视觉体系切换先行**：控制台在已实证的 shadcn 组件体系与 token 上生长，不再有双栈混搭期） |
